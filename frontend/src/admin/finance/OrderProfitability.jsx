...
export default function OrderProfitability({ days, from, to, bare = false }) {
  ...
  const SortBtn = ({ id, label, align = 'right' }) => {
    ...
    return (
      <button
        ...
        className={`inline-flex w-full items-center gap-1 ${align === 'right' ? 'justify-end' : ''} transition hover:text-[var(--od-text)] ${active ? 'text-[var(--od-text)]' : ''}`}
      >
        {label}
  ...
  return (
    <section className={bare ? undefined : "mb-10"}>
      ...
      {t && (
        <div className="adm-divide-x mb-4 grid grid-cols-2 border-y border-[var(--od-border-light)] sm:grid-cols-4">
          {[
            ...
            <div key={label} className="px-4 py-4">
              ...
              <p className="adm-metric mt-2 text-[18px] text-[var(--od-text)]">{value}</p>
            </div>
          ))}
        </div>
      )}

      <p className="mb-4 text-[12px] leading-relaxed text-[var(--adm-label)]">
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[860px] border-collapse text-[12px]">
          ...
          <tbody>
            {busy && !data ? (
              <tr><td colSpan={12} className="py-10 text-center text-[var(--adm-label)]">Loading…</td></tr>
            ) : !data || data.rows.length === 0 ? (
              <tr><td colSpan={12} className="py-10 text-center text-[var(--adm-label)]">No orders in this range.</td></tr>
            ) : data.rows.map((r) => {
              ...
              return (
                <Fragment key={r.id}>
                  <tr
                    ...
                    className="cursor-pointer border-b border-[var(--od-border-light)] adm-row-hover"
                  >
                    <td className="py-2.5 text-[var(--adm-label)]">
                      {expanded ? <ChevronDown size={13} /> : <ChevronRight size={13} />}
                    </td>
                    <td className="py-2.5 font-mono text-[12px] text-[var(--od-text)]">{r.orderNumber}</td>
                    <td className="max-w-[150px] truncate py-2.5 text-[var(--od-text)]">{r.customer}</td>
                    <td className="whitespace-nowrap py-2.5 text-[var(--adm-label)]">{fmtDate(r.date)}</td>
                    <td className="py-2.5 text-right tabular-nums text-[var(--od-text)]">{r.revenue.toLocaleString()}</td>
                    <td className="hidden py-2.5 text-right tabular-nums text-[var(--adm-label)] lg:table-cell">{r.cogs.toLocaleString()}</td>
                    <td className="hidden py-2.5 text-right tabular-nums text-[var(--adm-label)] xl:table-cell">{r.packaging.toLocaleString()}</td>
                    <td className="hidden py-2.5 text-right tabular-nums text-[var(--adm-label)] xl:table-cell">{r.courier.toLocaleString()}</td>
                    <td className="hidden py-2.5 text-right tabular-nums text-[var(--adm-label)] xl:table-cell">{r.paymentFee.toLocaleString()}</td>
                  ...
                  {expanded && (
                    <tr className="border-b border-[var(--od-border-light)] bg-[var(--od-hover)]">
                      <td />
                        ...
                        <div className="grid gap-6 md:grid-cols-2">
                          <div className="max-w-sm">
                            ...
                            <dl className="space-y-1.5">
                              ...
                              <div className="!mt-2 flex items-center justify-between border-t border-[var(--od-border)] pt-2">
                                <dt className="text-[12px] font-medium text-[var(--od-text)]">Net profit</dt>
                                <dd className="adm-metric text-[14px] text-[var(--od-text)]">
                                  {pkr(r.netProfit)} {r.revenue > 0 && <span className="text-[11px] text-[var(--adm-label)]">({r.margin}%)</span>}
                                </dd>
                              </div>
                            </dl>
                          </div>
                          <div className="space-y-2 text-[13px] text-[var(--adm-label)]">
                            <p><span className="text-[var(--adm-label)]">City:</span> {r.city || '—'}</p>
                            <p><span className="text-[var(--adm-label)]">Payment:</span> {r.paymentMethod}</p>
                            <p><span className="text-[var(--adm-label)]">Stage:</span> {r.stage || r.status}</p>
                            {(r.cancelled || r.returned) && (
                              <p className="border-y border-[var(--od-border-light)] py-2 text-[12px] leading-relaxed text-[var(--od-text)]">
                            ...
                            <Link to={`/admin/orders/${r.id}`} className="inline-flex items-center gap-1 pt-1 text-[11px] uppercase tracking-[0.14em] text-[var(--adm-label)] hover:text-[var(--od-text)]">
                              Open order →
                            </Link>
                          </div>
                        </div>
      ...
      {data && data.total > data.limit && (
        <div className="mt-4">
          <p className="mb-3 text-[11px] uppercase tracking-[0.14em] text-[var(--adm-label)]">
...
function Line({ label, value, strong }) {
  return (
    <div className="flex items-center justify-between gap-4">
      <dt className={`text-[13px] ${strong ? 'text-[var(--od-text)]' : 'text-[var(--adm-label)]'}`}>{label}</dt>
      <dd className="shrink-0 tabular-nums text-[var(--od-text)]">
