...
/* ============================================================================
 * One line in the bag — premium cart register.
 *
 * Desktop: 5-column grid — [product] [price] [qty] [total] [remove].
 * Mobile: image + name stacked, qty/price row, actions below.
 * ========================================================================== */
...
export default function CartLine({
  ...
  return (
    <li
      className={`grid grid-cols-[100px_minmax(0,1fr)] items-start gap-5 py-6 transition-opacity duration-300 md:grid-cols-[minmax(0,1fr)_120px_150px_120px_44px] md:items-center md:gap-6 ${removing ? 'opacity-0' : 'opacity-100'}`}
      aria-busy={removing || undefined}
    >
      {/* ── PRODUCT cell: image + name/meta (mobile: 2-col inside) ── */}
      <Link to={`/product/${line.slug}`}
        className={`group relative block overflow-hidden bg-[#F5F5F5] ${blocked ? 'opacity-55' : ''}`}
