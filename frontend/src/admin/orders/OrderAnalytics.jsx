...
function Kpi({ label, value, sub, tone }) {
  ...
  return (
    <div className="rounded-lg border border-neutral-200 bg-neutral-50/60 px-3 py-2.5">
      ...
      <p className={`mt-0.5 text-[12px] font-semibold tabular-nums ${toneCls}`}>{value}</p>
...
function Panel({ title, children }) {
  return (
    <div className="rounded-lg border border-neutral-200 p-3">
      <p className="mb-1.5 text-[13px] font-semibold uppercase tracking-wider text-neutral-500">{title}</p>
      {children}
    </div>
  );
}
