  ...
  return (
    <AdminLayout title="Categories">
      <div className="mb-5 flex justify-end">
        <button onClick={() => setEditing({ ...EMPTY })} className="inline-flex items-center gap-1.5 rounded-full bg-neutral-900 px-4 py-2 text-[11px] font-semibold text-white hover:bg-black !px-5 !py-2.5 !text-[11px]"><Plus size={14} /> Add Category</button>
      </div>
        ...
        {editing && (
          <div className="rounded-2xl border border-neutral-200 bg-white h-fit p-6 lg:sticky lg:top-8">
            ...
            <div className="space-y-4">
              ...
              <div><label className="mb-1 block text-[10px] font-bold uppercase tracking-wider text-neutral-500">Description</label><textarea className="w-full rounded-xl border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900 min-h-20" value={editing.description} onChange={(e) => setEditing({ ...editing, description: e.target.value })} /></div>
              ...
              <button onClick={save} disabled={busy || !editing.name} className="inline-flex items-center gap-1.5 rounded-full bg-neutral-900 px-4 py-2 text-[11px] font-semibold text-white hover:bg-black w-full"><Save size={14} /> {busy ? 'Saving…' : 'Save category'}</button>
            </div>
          </div>
        )}
      </div>
