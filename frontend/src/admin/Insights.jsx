  ...
  return (
    <AdminLayout title="Insights">
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => load()}
            disabled={busy}
            className="inline-flex items-center gap-1.5 rounded-full border border-neutral-200 bg-white px-3 py-1.5 text-[12px] font-semibold text-neutral-700 hover:bg-neutral-50 disabled:opacity-50"
          >
      ...
      <div className="mb-6 grid gap-4 md:grid-cols-3">
        <MiniKpi icon={Clock} label="Peak hour" value={`${String(peakHour.hour).padStart(2, '0')}:00`} sub={`${peakHour.orders} orders`} accent="#8F8F97" />
        <MiniKpi icon={RotateCcw} label="Repeat rate" value={`${d.repeat.rate}%`} sub={`${d.repeat.repeat} of ${d.repeat.total} buyers came back`} accent="#D6D6DA" />
        <MiniKpi icon={MapPin} label="Top city" value={d.topCities[0]?._id || '—'} sub={d.topCities[0] ? `${pkr(d.topCities[0].revenue)} · ${d.topCities[0].orders} orders` : ''} accent="#A1A1AA" />
      </div>
      ...
      <div className="mb-6 grid gap-6 lg:grid-cols-2">
        <section className="rounded-2xl border border-neutral-200 bg-white p-6">
          <div className="mb-4 flex items-center justify-between">
            ...
            <span className="grid h-9 w-9 place-items-center rounded-xl bg-neutral-100 text-neutral-700"><MapPin size={15}/></span>
          </div>
                ...
                return (
                  <div key={c._id + i} className="rounded-xl border border-neutral-100 bg-neutral-50/60 p-3">
                    ...
                    <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-white">
                      <div className="h-full rounded-full bg-neutral-500 transition-all" style={{ width: `${w}%` }} />
                    </div>
                  </div>
                );
              })}
      ...
      <section className="rounded-2xl border border-neutral-200 bg-white p-6">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <p className="text-[12px] font-bold uppercase tracking-widest text-neutral-500">Cohort analysis</p>
            ...
            <table className="w-full min-w-[600px] text-[13px]">
              <thead>
                <tr className="border-b border-neutral-200 text-left">
                  <th className="py-2 text-[13px] font-bold uppercase tracking-widest text-neutral-500">First-order month</th>
                  <th className="py-2 text-right text-[13px] font-bold uppercase tracking-widest text-neutral-500">New customers</th>
                  <th className="py-2 text-right text-[13px] font-bold uppercase tracking-widest text-neutral-500">Repeat buyers</th>
                  <th className="py-2 text-right text-[13px] font-bold uppercase tracking-widest text-neutral-500">Repeat %</th>
                  <th className="py-2 text-right text-[13px] font-bold uppercase tracking-widest text-neutral-500">Total spent</th>
                </tr>
              </thead>
              <tbody>
                {d.cohort.map((c) => {
