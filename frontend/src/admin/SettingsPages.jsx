  ...
  return (
    <div className="sticky bottom-4 z-30 mt-6 flex items-center justify-between gap-4 rounded-md border border-neutral-900 bg-neutral-900 px-4 py-3 text-white shadow-md">
      ...
      <div className="flex items-center gap-2">
        <button onClick={onReset} className="rounded-md border border-white/20 px-3 py-1.5 text-[12px] font-semibold text-white/80 transition hover:bg-white/10">Discard</button>
        <button onClick={onSave} disabled={busy} className="inline-flex items-center gap-1.5 rounded-md bg-white px-4 py-1.5 text-[12px] font-semibold text-neutral-900 transition hover:bg-neutral-100 disabled:opacity-60">
...
export function SettingsStore() {
  ...
  if (!s) return <AdminLayout title="Store details"><div className="animate-pulse rounded-md bg-neutral-100 h-96 w-full" /></AdminLayout>;
      ...
      <div className="space-y-5">
        <Section title="Identity" description="These appear in the header, footer, and every automated email.">
          <div className="grid gap-4 md:grid-cols-2">
            <div><label className="mb-1 block text-[13px] font-bold uppercase tracking-wider text-neutral-500">Store name</label><input className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900" value={s.storeName || ''} onChange={(e) => set('storeName', e.target.value)} /></div>
            <div><label className="mb-1 block text-[13px] font-bold uppercase tracking-wider text-neutral-500">Tagline</label><input className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900" value={s.tagline || ''} onChange={(e) => set('tagline', e.target.value)} /></div>
          </div>
        </Section>

        <Section title="Contact" description="Shown in the footer, order confirmations, and used by customers to reach you.">
          <div className="grid gap-4 md:grid-cols-2">
            <div><label className="mb-1 block text-[13px] font-bold uppercase tracking-wider text-neutral-500">Contact email</label><input className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900" type="email" value={s.contactEmail || ''} onChange={(e) => set('contactEmail', e.target.value)} placeholder="care@yourstore.com" /></div>
            <div><label className="mb-1 block text-[13px] font-bold uppercase tracking-wider text-neutral-500">Contact phone</label><input className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900" value={s.contactPhone || ''} onChange={(e) => set('contactPhone', e.target.value)} placeholder="+92 300 1234567" /></div>
          </div>
        </Section>
          ...
          <div className="space-y-3">
            {(s.trustBadges || []).map((b, i) => (
              <div key={i} className="grid gap-2 md:grid-cols-[220px_1fr]">
                <input className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900 !py-2 !text-[12px] font-semibold" value={b.title || ''} onChange={(e) => setBadge(i, 'title', e.target.value)} placeholder="Badge title" />
                <input className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900 !py-2 !text-[12px]" value={b.text || ''} onChange={(e) => setBadge(i, 'text', e.target.value)} placeholder="Short supporting text" />
              </div>
            ))}
          </div>
        </Section>
...
export function SettingsPayments() {
  const { s, setS, dirty, reset, save, busy } = useSettingsSlice();
  if (!s) return <AdminLayout title="Payments"><div className="animate-pulse rounded-md bg-neutral-100 h-96 w-full" /></AdminLayout>;
      ...
      <div className="space-y-5">
        ...
        <Section title="Mobile wallets" description="Customer transfers money to your account, then enters the transaction ID at checkout.">
          <div className="space-y-3">
            ...
            />
            {pm.jazzcash && (
              <div className="ml-4 grid gap-3 rounded-md border border-neutral-200 bg-neutral-50 p-4 md:grid-cols-2">
                <div><label className="mb-1 block text-[13px] font-bold uppercase tracking-wider text-neutral-500">JazzCash number</label><input className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900 font-mono" value={pm.jazzcashNumber || ''} onChange={(e) => setPM('jazzcashNumber', e.target.value)} placeholder="0300 1234567" /></div>
                <div><label className="mb-1 block text-[13px] font-bold uppercase tracking-wider text-neutral-500">Account title</label><input className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900" value={pm.jazzcashTitle || ''} onChange={(e) => setPM('jazzcashTitle', e.target.value)} placeholder="Your Name" /></div>
              </div>
            )}
            ...
            {pm.easypaisa && (
              <div className="ml-4 grid gap-3 rounded-md border border-neutral-200 bg-neutral-50 p-4 md:grid-cols-2">
                <div><label className="mb-1 block text-[13px] font-bold uppercase tracking-wider text-neutral-500">EasyPaisa number</label><input className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900 font-mono" value={pm.easypaisaNumber || ''} onChange={(e) => setPM('easypaisaNumber', e.target.value)} placeholder="0345 1234567" /></div>
                <div><label className="mb-1 block text-[13px] font-bold uppercase tracking-wider text-neutral-500">Account title</label><input className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900" value={pm.easypaisaTitle || ''} onChange={(e) => setPM('easypaisaTitle', e.target.value)} placeholder="Your Name" /></div>
              </div>
            )}
          </div>
        </Section>
        ...
        <Section title="Bank transfer" description="For customers who prefer a direct bank transfer.">
          ...
          {pm.bank && (
            <div className="mt-3 rounded-md border border-neutral-200 bg-neutral-50 p-4">
              ...
              <textarea className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900 min-h-28 font-mono text-[12px]" value={pm.bankDetails || ''} onChange={(e) => setPM('bankDetails', e.target.value)} placeholder={'Bank: Meezan Bank\nTitle: Your Business Name\nIBAN: PK00 MEZN 0000 0000 0000 0000'} />
        ...
        <Section title="Online card gateway" description="Accept Visa / Mastercard payments online. Enter your provider keys — sandbox first, switch to live when you are ready to launch. Customers see cards at checkout only once a gateway is configured and enabled.">
          ...
          <div className="rounded-md border border-neutral-200 bg-neutral-50 p-4">
            ...
            {sp.sandbox && (
              <p className="mt-2 rounded-md bg-[#F6F1E6] px-3 py-2 text-[12px] text-[#6B552F]">
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-2">
              <div>
                ...
                <input className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 font-mono text-[12px] outline-none transition focus:border-neutral-900" value={sp.apiKey || ''} onChange={(e) => setSP('apiKey', e.target.value)} placeholder="SF-XXXX-…" />
              </div>
              <div>
                <label className="mb-1 block text-[12px] font-bold uppercase tracking-wider text-neutral-500">Secret key</label>
                <input type="password" className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 font-mono text-[12px] outline-none transition focus:border-neutral-900" value={sp.secret || ''} onChange={(e) => setSP('secret', e.target.value)} placeholder="••••••••" />
              </div>
            </div>
          ...
          <div className="rounded-md border border-neutral-200 bg-neutral-50 p-4">
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-3">
              <div>
                ...
                <input className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 font-mono text-[12px] outline-none transition focus:border-neutral-900" value={jz.merchantId || ''} onChange={(e) => setJZ('merchantId', e.target.value)} placeholder="MC-…" />
              </div>
              <div>
                <label className="mb-1 block text-[12px] font-bold uppercase tracking-wider text-neutral-500">Password</label>
                <input type="password" className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 font-mono text-[12px] outline-none transition focus:border-neutral-900" value={jz.password || ''} onChange={(e) => setJZ('password', e.target.value)} placeholder="••••••••" />
              </div>
              <div>
                <label className="mb-1 block text-[12px] font-bold uppercase tracking-wider text-neutral-500">Integrity salt</label>
                <input type="password" className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 font-mono text-[12px] outline-none transition focus:border-neutral-900" value={jz.integritySalt || ''} onChange={(e) => setJZ('integritySalt', e.target.value)} placeholder="••••••••" />
              </div>
            </div>
            <p className="mt-2 text-[12px] text-neutral-500">
...
export function SettingsShipping() {
  const { s, setS, dirty, reset, save, busy } = useSettingsSlice();
  if (!s) return <AdminLayout title="Shipping"><div className="animate-pulse rounded-md bg-neutral-100 h-96 w-full" /></AdminLayout>;
        ...
        <Section title="Shipping rates" description="Flat rate applies to every order below the free-shipping threshold.">
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <input className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900" type="number" min="0" value={s.shippingFlatRate ?? 350} onChange={(e) => set('shippingFlatRate', Number(e.target.value))} />
            ...
            <div>
              ...
              <input className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900" type="number" min="0" value={s.freeShippingThreshold ?? 4999} onChange={(e) => set('freeShippingThreshold', Number(e.target.value))} />
        ...
        <Section title="Per-order operating costs" description="These are subtracted from your gross profit — set them for accurate P&L.">
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <input className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900" type="number" min="0" value={oc.packingPerOrder || 0} onChange={(e) => setOC('packingPerOrder', e.target.value)} placeholder="e.g. 40" />
            ...
            <div>
              ...
              <input className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900" type="number" min="0" value={oc.shippingSubsidy || 0} onChange={(e) => setOC('shippingSubsidy', e.target.value)} placeholder="e.g. 50" />
        ...
        <Section title="Courier cost per parcel" description="What the courier bills you. Used by Finance → Order profitability to work out what you keep on each order.">
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <input className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900" type="number" min="0" value={oc.defaultCourierCost || 0} onChange={(e) => setOC('defaultCourierCost', e.target.value)} placeholder="e.g. 250" />
            ...
            <div>
              ...
              <input className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900" type="number" min="1" step="0.5" value={oc.returnCourierMultiplier ?? 2} onChange={(e) => setOC('returnCourierMultiplier', e.target.value)} />
            ...
            <div className="mt-1.5 space-y-2">
              {(oc.courierByCity || []).map((row, i) => (
                <div key={i} className="flex items-center gap-2">
                  <input
                    className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900 flex-1" placeholder="City, e.g. Karachi" value={row.city || ''}
                    onChange={(e) => setS({ ...s, operatingCosts: { ...oc, courierByCity: oc.courierByCity.map((r, j) => (j === i ? { ...r, city: e.target.value } : r)) } })}
                  />
                  <input
                    className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900 w-32" type="number" min="0" placeholder="PKR" value={row.cost ?? ''}
                  ...
                  <button
                    ...
                    className="shrink-0 rounded-md px-3 py-2 text-[12px] font-semibold text-neutral-500 hover:bg-[#F5EDEB] hover:text-[#9A5548]"
                  >Remove</button>
                </div>
              ))}
              <button
                onClick={() => setS({ ...s, operatingCosts: { ...oc, courierByCity: [...(oc.courierByCity || []), { city: '', cost: 0 }] } })}
                className="w-full rounded-md border border-dashed border-neutral-300 py-2 text-[13px] font-semibold text-neutral-600 hover:border-neutral-400 hover:text-neutral-900"
          ...
          <div className="grid gap-4 md:grid-cols-3">
            {[
              ...
              <div key={key}>
                ...
                <input
                  className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900" type="number" min="0" step="0.05" placeholder={ph}
        ...
        <Section title="Targets & thresholds" description="Drive the dashboard goal tracker and the profitability warnings.">
          <div className="grid gap-4 md:grid-cols-2">
            <div>
              ...
              <input className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900" type="number" min="0" value={s.monthlyRevenueGoal || 0} onChange={(e) => set('monthlyRevenueGoal', Number(e.target.value) || 0)} placeholder="e.g. 500000" />
            ...
            <div>
              ...
              <input className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900" type="number" min="0" max="100" value={s.marginThresholdPercent ?? 15} onChange={(e) => set('marginThresholdPercent', Number(e.target.value) || 0)} placeholder="e.g. 15" />
        ...
        <Section title="Monthly marketing & fixed costs" description="Fixed costs you pay every month — divided across all monthly orders in the P&L view.">
          <div className="grid gap-4 md:grid-cols-3">
            <div>
              ...
              <input className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900" type="number" min="0" value={oc.monthlyMarketing || 0} onChange={(e) => setOC('monthlyMarketing', e.target.value)} placeholder="e.g. 25000" />
            ...
            <div>
              ...
              <input className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900" type="number" min="0" value={oc.monthlySeo || 0} onChange={(e) => setOC('monthlySeo', e.target.value)} placeholder="e.g. 10000" />
            ...
            <div>
              ...
              <input className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900" type="number" min="0" value={oc.monthlyOther || 0} onChange={(e) => setOC('monthlyOther', e.target.value)} placeholder="e.g. 5000" />
          ...
          <form onSubmit={submitUsername} className="space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="relative">
                ...
                <input
                  className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900 pr-10"
        ...
        <Section title="Change password" description="A strong password uses at least 12 characters with upper- and lower-case letters, numbers, and a symbol.">
          <form onSubmit={submit} className="space-y-4">
            <div className="relative">
              ...
              <input className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900 pr-10" type={show.c ? 'text' : 'password'} value={current} onChange={(e) => setCurrent(e.target.value)} autoComplete="current-password" />
              {eyeBtn('c')}
            </div>
            <div className="grid gap-4 md:grid-cols-2">
              <div className="relative">
                <label className="mb-1 block text-[13px] font-bold uppercase tracking-wider text-neutral-500">New password</label>
                <input className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900 pr-10" type={show.n ? 'text' : 'password'} value={next} onChange={(e) => setNext(e.target.value)} autoComplete="new-password" placeholder="Min 8 characters, letters + numbers" />
                {eyeBtn('n')}
                {next && (
                  <div className="mt-2">
              ...
              <div className="relative">
                ...
                <input className="w-full rounded-md border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900 pr-10" type={show.x ? 'text' : 'password'} value={confirm} onChange={(e) => setConfirm(e.target.value)} autoComplete="new-password" />
                {eyeBtn('x')}
  ...
  return (
    <AdminLayout title="Legal & Policies">
      ...
      <div className="rounded-md border border-dashed border-neutral-300 bg-white p-10 text-center">
        <span className="mx-auto grid h-14 w-14 place-items-center rounded-md bg-neutral-100 text-neutral-500">
          <Sparkles size={20} />
        </span>
