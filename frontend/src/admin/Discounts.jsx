...
export default function Discounts() {
  ...
  return (
    <AdminLayout title="Discounts">
      ...
      {!list ? <div className="animate-pulse rounded-xl bg-neutral-100 h-40 rounded-2xl" /> : list.length === 0 ? <div className="grid place-items-center rounded-2xl border border-dashed border-neutral-200 bg-white py-16 text-center"><BadgePercent size={32} className="mb-3 text-neutral-300" /><p className="text-[12px] font-medium text-neutral-700">No discount codes yet</p><p className="mt-1 text-[12px] text-neutral-500">Create your first code, e.g. WELCOME10.</p><button onClick={openNew} className="mt-4 inline-flex items-center gap-1.5 rounded-full bg-neutral-900 px-4 py-2 text-[12px] font-semibold text-white hover:bg-black"><Plus size={13} /> Create code</button></div> : filtered.length === 0 ? <div className="grid place-items-center rounded-2xl border border-dashed border-neutral-200 bg-white py-16 text-center"><Search size={28} className="mb-3 text-neutral-300" /><p className="text-[12px] font-medium text-neutral-700">No codes match "{q}"</p></div> : (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((d) => (
            <div key={d._id} className={`group rounded-2xl border bg-white p-5 transition hover:shadow-sm ${d.active ? 'border-neutral-200' : 'border-neutral-200 bg-neutral-50/50'}`}>
              ...
              <div className="mt-4">
                <div className="flex items-center justify-between text-[12px]"><span className="font-medium text-neutral-600">{d.usedCount || 0} use{(d.usedCount || 0) === 1 ? '' : 's'}</span>{d.maxUses > 0 && <span className="text-neutral-500">/{d.maxUses} max</span>}</div>
