        ...
        <div className="space-y-5">
          ...
          {(d?.byPromotion || []).length > 0 && (
            <section className="rounded-2xl border border-neutral-200 bg-white p-5">
              <p className="text-[11px] font-bold uppercase tracking-widest text-neutral-600">Top promotions</p>
          ...
          {bundles.length > 0 && (
            <section className="rounded-2xl border border-neutral-200 bg-white p-5">
              <p className="text-[11px] font-bold uppercase tracking-widest text-neutral-600">Top bundles</p>
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            <section className="rounded-2xl border border-neutral-200 bg-white p-5">
              <p className="text-[11px] font-bold uppercase tracking-widest text-neutral-600">Starting soon</p>
              {!upcoming.length ? (
            ...
            <section className="rounded-2xl border border-neutral-200 bg-white p-5">
              <p className="text-[11px] font-bold uppercase tracking-widest text-neutral-600">Finished</p>
                ...
                <ul className="mt-4 space-y-2">
                  {expired.map((p) => (
                    <li key={p._id}>
                      <Link to={`/admin/promotions/${p._id}`} className="flex min-h-[44px] items-center justify-between gap-3 rounded-xl border border-neutral-200 px-4 py-2.5 transition hover:border-neutral-300">
                        <span className="min-w-0 truncate text-[13px] text-neutral-900">{p.name}</span>
                        <span className="shrink-0 text-[11px] text-neutral-600">{num(p.usedCount)} uses</span>
                      </Link>
                    </li>
                  ))}
                </ul>
          ...
          {(d?.daily || []).length > 0 && (
            <section className="rounded-2xl border border-neutral-200 bg-white p-5">
              <p className="text-[11px] font-bold uppercase tracking-widest text-neutral-600">Day by day</p>
              <ul className="mt-4 space-y-1.5">
                {d.daily.slice(-14).map((row) => {
                  const max = Math.max(...d.daily.map((x) => x.given), 1);
                  return (
                    <li key={row.date} className="flex items-center gap-3">
                      <span className="w-20 shrink-0 text-[11px] tabular-nums text-neutral-600">
                      ...
                      <span className="w-28 shrink-0 text-right text-[11px] tabular-nums text-neutral-600">
