    ...
    <div className="rounded-2xl border border-neutral-200 bg-white p-5">
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-[11px] font-bold uppercase tracking-widest text-neutral-500">Monthly revenue goal</p>
          <p className="mt-1 text-[12px] text-neutral-500">
            {goal.daysRemaining} day{goal.daysRemaining === 1 ? '' : 's'} left in the month
          </p>
        </div>
        <div className="flex shrink-0 items-center gap-2">
          <span className={`rounded-full px-2.5 py-1 text-[10.5px] font-bold ring-1 ${pace.cls}`}>{pace.label}</span>
          <button
      ...
      {editing ? (
        ...
        <button onClick={() => { setValue(''); setEditing(true); }}
          className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl border border-dashed border-neutral-300 py-4 text-[12.5px] font-semibold text-neutral-600 transition hover:border-neutral-400 hover:text-neutral-900">
        ...
        <>
          ...
          {goal.dailyNeeded > 0 && (
            <p className="mt-3 rounded-lg bg-neutral-50 px-3 py-2 text-[11.5px] leading-relaxed text-neutral-600">
