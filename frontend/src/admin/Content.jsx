    ...
    <AdminLayout title="Content">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-neutral-200 bg-white p-6">
            ...
            <div className="mb-6 rounded-2xl border border-neutral-200 bg-white/40 p-4">
              ...
              <div className="space-y-3">
                ...
                <div><label className="mb-1 block text-[13px] font-bold uppercase tracking-wider text-neutral-500">Subtitle</label><textarea className="w-full rounded-xl border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900 min-h-20" value={hero.subtitle || ''} onChange={(e) => setHero('subtitle', e.target.value)} placeholder="A short description shown below the title" /></div>
              </div>
            </div>
            ...
            <div className="mb-6 rounded-2xl border border-neutral-200 bg-white/40 p-4">
              ...
              {hero.showButtons === false ? (
                ...
                <>
                  <div className="mb-3">
                    <label className="mb-1 block text-[13px] font-bold uppercase tracking-wider text-neutral-500">Button style</label>
                    <div className="grid grid-cols-2 gap-2">
                      <button type="button" onClick={() => setHero('ctaStyle', 'buttons')}
                        ...
                        <span className={`mt-0.5 block text-[13px] ${hero.ctaStyle !== 'dropdown' ? 'text-white/70' : 'text-neutral-500'}`}>Women + Men (classic)</span>
                      </button>
                      <button type="button" onClick={() => setHero('ctaStyle', 'dropdown')}
                        className={`rounded-xl border px-3 py-2.5 text-left text-xs transition ${hero.ctaStyle === 'dropdown' ? 'border-obsidian bg-neutral-900 text-white' : 'border-neutral-200 bg-white hover:bg-neutral-100'}`}>
                        <span className="block font-semibold">Dropdown Menu</span>
                        <span className={`mt-0.5 block text-[13px] ${hero.ctaStyle === 'dropdown' ? 'text-white/70' : 'text-neutral-500'}`}>One Shop button with menu</span>
                      </button>
                    </div>
                  </div>
                  ...
                  {hero.ctaStyle === 'dropdown' ? (
                    <>
                      <div><label className="mb-1 block text-[13px] font-bold uppercase tracking-wider text-neutral-500">Button text</label><input className="w-full rounded-xl border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900" value={hero.ctaWomen || ''} onChange={(e) => setHero('ctaWomen', e.target.value)} placeholder="Shop Now" /></div>
                      <div className="mt-3">
                        <label className="mb-1 block text-[13px] font-bold uppercase tracking-wider text-neutral-500">Dropdown menu items</label>
                        <p className="mt-1 text-[12px] text-neutral-500">One item per line — format: <code className="rounded bg-neutral-100 px-1">Label | /link</code></p>
            ...
            <div className="mb-6 rounded-2xl border border-neutral-200 bg-white/40 p-4">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <p className="mt-1 text-[12px] text-neutral-500">Recommended: 1920 × 1080 (16:9). Upload a high-quality JPG or WebP.</p>
                </div>
                <div>
                  <label className="mb-1 block text-[13px] font-bold uppercase tracking-wider text-neutral-500">Banner video (optional)</label>
                  <MediaPicker value={hero.video || ''} onChange={(v) => setHero('video', v)} accept="video" buttonText="Upload video" hideUrl />
                  <p className="mt-1 text-[12px] text-neutral-500">If a video is uploaded, it replaces the image. Autoplays silently and loops. MP4 or WebM.</p>
                </div>
              </div>
            </div>
              ...
              <div className="space-y-4">
                <div>
                  <div className="mb-1 flex items-center justify-between">
                    ...
                    <span className="rounded-full bg-neutral-900 px-2 py-0.5 text-[13px] font-bold text-white">{hero.overlayOpacity ?? 40}%</span>
                  </div>
                  ...
                  <p className="mt-1 text-[12px] text-neutral-500">Adds a dark gradient over the image so text stays readable. Higher = darker.</p>
                </div>

                <div>
                  <label className="mb-1 block text-[13px] font-bold uppercase tracking-wider text-neutral-500">Text position</label>
          ...
          <div className="rounded-2xl border border-neutral-200 bg-white p-6">
            ...
            <div className={`mt-6 space-y-6 ${split.enabled === false ? 'pointer-events-none opacity-40' : ''}`}>
              ...
              <div className="grid gap-3 md:grid-cols-2">
                <div><label className="mb-1 block text-[13px] font-bold uppercase tracking-wider text-neutral-500">Eyebrow (small caps)</label><input className="w-full rounded-xl border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900" value={split.eyebrow || ''} onChange={(e) => setSplit('eyebrow', e.target.value)} placeholder="The Signature Edit" /></div>
                <div><label className="mb-1 block text-[13px] font-bold uppercase tracking-wider text-neutral-500">Title (Enter for new line)</label><textarea className="w-full rounded-xl border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900 min-h-16" value={split.title || ''} onChange={(e) => setSplit('title', e.target.value)} placeholder={'Premium,\nperfected.'} /></div>
                <div className="md:col-span-2"><label className="mb-1 block text-[13px] font-bold uppercase tracking-wider text-neutral-500">Subtitle</label><textarea className="w-full rounded-xl border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900 min-h-16" value={split.subtitle || ''} onChange={(e) => setSplit('subtitle', e.target.value)} /></div>
              </div>
              ...
              <div className="grid gap-3 md:grid-cols-4">
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <input type="color" className="h-10 w-14 cursor-pointer rounded-lg border border-neutral-200" value={split.textColor || '#FFFFFF'} onChange={(e) => setSplit('textColor', e.target.value)} />
                    <input className="w-full rounded-xl border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900 font-mono text-xs" value={split.textColor || '#FFFFFF'} onChange={(e) => setSplit('textColor', e.target.value)} />
                  </div>
                </div>
                <div>
                  <label className="mb-1 block text-[13px] font-bold uppercase tracking-wider text-neutral-500">Title font</label>
                ...
                <div>
                  <label className="mb-1 block text-[13px] font-bold uppercase tracking-wider text-neutral-500">Text shadow</label>
                ...
                <div>
                  <label className="mb-1 block text-[13px] font-bold uppercase tracking-wider text-neutral-500">Overlay strength ({split.overlayOpacity ?? 25}%)</label>
          ...
          <div className="rounded-2xl border border-neutral-200 bg-white p-6">
            ...
            <div className={`mt-4 ${marquee.enabled !== false ? '' : 'pointer-events-none opacity-40'}`}>
              <label className="mb-1 block text-[13px] font-bold uppercase tracking-wider text-neutral-500">Items — one per line</label>
          ...
          <div className="rounded-2xl border border-neutral-200 bg-white p-6">
            ...
            <div className={`mt-4 grid gap-4 ${cookie.enabled !== false ? '' : 'pointer-events-none opacity-40'}`}>
              <div><label className="mb-1 block text-[13px] font-bold uppercase tracking-wider text-neutral-500">Popup title</label><input className="w-full rounded-xl border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900" value={cookie.title || ''} onChange={(e) => setCookie('title', e.target.value)} /></div>
              <div><label className="mb-1 block text-[13px] font-bold uppercase tracking-wider text-neutral-500">Popup text</label><textarea className="w-full rounded-xl border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900 min-h-20" value={cookie.text || ''} onChange={(e) => setCookie('text', e.target.value)} /></div>
            </div>
          </div>

          <button onClick={save} disabled={busy} className="inline-flex items-center gap-1.5 rounded-full bg-neutral-900 px-4 py-2 text-[12px] font-semibold text-white hover:bg-black w-full lg:w-auto">{busy ? 'Saving…' : 'Save All Changes'}</button>
        </div>

        {/* Live preview */}
        <div className="space-y-6 lg:sticky lg:top-8 lg:self-start">
          <div className="rounded-2xl border border-neutral-200 bg-white overflow-hidden">
            <p className="border-b border-neutral-200 bg-neutral-100 px-5 py-2.5 text-[13px] font-semibold uppercase tracking-widest text-neutral-500">Live Preview</p>
            {offer.enabled && (
              <div className="flex items-center justify-center gap-2 bg-neutral-900 px-4 py-2 text-center text-[12px] text-white">
            ...
            {hero.fullScreen ? (
              <div className="relative h-72 overflow-hidden">
                ...
                <div className={`absolute inset-x-0 bottom-0 p-6 ${hero.align === 'center' ? 'text-center' : ''}`}>
                  ...
                  <p className={`mt-1.5 max-w-md text-[12px] leading-relaxed text-white/70 ${hero.align === 'center' ? 'mx-auto' : ''}`}>{hero.subtitle}</p>
                  <div className={`mt-3 flex gap-2 ${hero.align === 'center' ? 'justify-center' : ''}`}>
                    <span className="rounded-full bg-white px-3 py-1.5 text-[13px] font-semibold uppercase tracking-widest text-black">{hero.ctaWomen}</span>
                    <span className="rounded-full border border-white/50 px-3 py-1.5 text-[13px] font-semibold uppercase tracking-widest text-white">{hero.ctaMen}</span>
                  </div>
                </div>
              </div>
            ) : (
              <div className="grid items-center gap-5 p-6 sm:grid-cols-2">
                <div>
                  ...
                  <div className="mt-4 flex gap-2">
                    <span className="inline-flex items-center gap-1.5 rounded-full bg-neutral-900 px-4 py-2 text-[12px] font-semibold text-white hover:bg-black !px-3 !py-2 text-[12px]">{hero.ctaWomen} <ArrowRight size={11} /></span>
                    <span className="inline-flex items-center gap-1.5 rounded-full border border-neutral-300 bg-white px-4 py-2 text-[12px] font-semibold text-neutral-700 hover:bg-neutral-50 !px-3 !py-2 text-[12px]">{hero.ctaMen}</span>
                  </div>
                </div>
                {hero.image
            ...
            {marquee.enabled !== false && (marquee.items || []).filter(Boolean).length > 0 && (
              <div className="overflow-hidden bg-neutral-900 py-2 text-white">
                <p className="whitespace-nowrap text-center text-[13px] font-medium uppercase tracking-widest opacity-90">
      ...
      <div className="mt-6 card p-6">
        <div className="flex items-start gap-3">
          ...
          <a href="/admin/faq" className="inline-flex items-center gap-1.5 rounded-full border border-neutral-300 bg-white px-4 py-2 text-[12px] font-semibold text-neutral-700 hover:bg-neutral-50 text-xs">Manage FAQ →</a>
        </div>
      </div>
    </AdminLayout>
  );
