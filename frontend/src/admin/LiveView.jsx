  ...
  return (
    <AdminLayout title="Live View">
      ...
      <section>
        ...
        {d.feed.length === 0 ? (
          <p className="border-y border-white/10 py-10 text-center text-[12px] text-white/35">
            Abhi koi activity nahi — store khulte hi yahan real-time feed chalegi.
          </p>
        ) : (
          <div className="overflow-x-auto">
            <div className="hidden border-b border-white/10 py-2 md:grid md:grid-cols-[0.7fr_1.1fr_0.6fr_minmax(0,1.4fr)_0.6fr] md:gap-3">
              {['Time', 'Event', 'Device', 'Page / context', 'City'].map((h) => <p key={h} className="adm-label">{h}</p>)}
            </div>
            {d.feed.map((e, i) => (
              <div key={i} className="grid grid-cols-2 items-center gap-2 border-b border-white/5 py-3 md:grid-cols-[0.7fr_1.1fr_0.6fr_minmax(0,1.4fr)_0.6fr] md:gap-3">
                <span className="text-[11px] tabular-nums text-white/40">{ago(e.createdAt)}</span>
                <span className="text-[12px] text-white">{EV_LABEL[e.event] || e.event}</span>
                <span className="text-[11px] uppercase tracking-[0.12em] text-white/35">{e.device || '—'}</span>
                <span className="truncate font-mono text-[11px] text-white/50">{e.path}</span>
                <span className="text-[11px] text-white/35">{e.city || '—'}</span>
              </div>
            ))}
          </div>
        )}
