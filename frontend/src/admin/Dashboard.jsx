  ...
  const inner = (
    <>
      <div className="flex items-center justify-between">
        ...
        {change !== 0 && <span className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-[9px] font-bold ${positive ? 'bg-emerald-50 text-emerald-700' : negative ? 'bg-red-50 text-red-700' : 'bg-neutral-100 text-neutral-600'}`}>{positive ? <ArrowUpRight size={11} /> : negative ? <ArrowDownRight size={11} /> : null}{changeText}</span>}
      </div>
      ...
      <p className="mt-1 text-[9px] text-neutral-400">{compareLabel}</p>
...
function StockRow({ product: p, onSaved }) {
  ...
  if (editing) return (
    <div className="flex items-center gap-2 rounded-lg bg-neutral-50 p-1.5">
      ...
      <input type="number" min="0" autoFocus value={value} onChange={(e) => setValue(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') save(); if (e.key === 'Escape') setEditing(false); }} className="w-full min-w-0 rounded-md border border-neutral-300 px-2 py-1.5 text-[9px] tabular-nums outline-none focus:border-neutral-900" />
      <button onClick={save} disabled={busy} className="shrink-0 rounded-md bg-neutral-900 px-2.5 py-1.5 text-[9px] font-semibold text-white disabled:opacity-50">Save</button>
      <button onClick={() => setEditing(false)} className="shrink-0 rounded-md px-1.5 py-1.5 text-[9px] font-semibold text-neutral-500 hover:text-neutral-900">Cancel</button>
    </div>
  );
  return (
    <div className="group flex items-center gap-3 rounded-lg p-1.5 transition hover:bg-neutral-50">
      <Link to={`/admin/products/${p._id}`} className="flex min-w-0 flex-1 items-center gap-3"><Img src={p.images?.[0]?.url} alt="" className="h-10 w-8 shrink-0 rounded-md border border-neutral-200 object-cover" /><span className="line-clamp-2 flex-1 text-[9px] font-medium text-neutral-800">{p.name}</span></Link>
...
function PipelineStrip({ stats }) {
  ...
  return (
    <div className="rounded-2xl border border-neutral-200 bg-white p-5">
      <div className="flex items-center justify-between">
        <div><p className="text-[9px] font-bold uppercase tracking-widest text-neutral-500">Order pipeline</p><p className="mt-1 text-[9px] text-neutral-500">Where every order is right now</p></div>
        <Link to="/admin/orders" className="text-[9px] font-semibold text-neutral-500 hover:text-neutral-900">Manage all →</Link>
      </div>
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
        {items.map((it) => (
          <Link key={it.label} to={it.to} className="group flex items-center gap-2 rounded-lg p-2 transition hover:bg-neutral-50">
            ...
            <div className="min-w-0 flex-1"><p className="truncate text-[9px] text-neutral-500">{it.label}</p><p className={`text-[10px] font-bold tabular-nums ${it.text}`}>{it.n}</p></div>
          </Link>
        ))}
      </div>
      {stats.pending > 0 && (
        <Link to="/admin/orders?group=new" className="mt-3 flex items-center justify-between gap-3 rounded-xl border border-amber-200 bg-amber-50 px-3.5 py-2.5 transition hover:border-amber-300 hover:bg-amber-100">
          <span className="min-w-0 text-[9px] font-medium text-amber-900"><b className="tabular-nums">{stats.pending}</b> new order{stats.pending === 1 ? '' : 's'} waiting to be confirmed</span>
          <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-amber-900 px-3 py-1 text-[9px] font-semibold text-white">Review now <ChevronRight size={11} /></span>
        </Link>
      )}
    </div>
  );
  ...
  return (
    <div className="rounded-2xl border border-neutral-200 bg-white p-6">
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div><p className="text-[9px] font-bold uppercase tracking-widest text-neutral-500">Last 14 days</p><p className="mt-1 font-sans text-2xl font-semibold tabular-nums text-neutral-900">{mode === 'revenue' ? pkr(total) : total.toLocaleString()}</p></div>
        <div className="flex items-center gap-1 rounded-full border border-neutral-200 bg-neutral-50 p-1">
          {['revenue', 'orders'].map((m) => (
            <button key={m} onClick={() => setMode(m)} className={`rounded-full px-3 py-1 text-[9px] font-semibold uppercase tracking-wider transition ${mode === m ? 'bg-neutral-900 text-white' : 'text-neutral-500 hover:text-neutral-900'}`}>{m}</button>
          ))}
        </div>
      </div>
...
function StatusDonut({ byStatus }) {
  ...
  return (
    <div className="rounded-2xl border border-neutral-200 bg-white p-6">
      <p className="text-[9px] font-bold uppercase tracking-widest text-neutral-500">Order status mix</p>
      {total === 0 ? <p className="py-16 text-center text-sm text-neutral-400">No orders yet.</p> : (
        <div className="mt-4 flex items-center gap-4">
          ...
          <ul className="flex-1 space-y-1.5">{data.slice(0, 6).map((d) => <li key={d.name} className="flex items-center gap-2 text-[9px]"><span className="h-2 w-2 rounded-full" style={{ background: d.color }} /><span className="flex-1 text-neutral-600">{d.name}</span><span className="font-semibold tabular-nums text-neutral-900">{d.value}</span></li>)}</ul>
        </div>
      )}
    </div>
  );
...
function TodayHourly({ hourly }) {
  ...
  return (
    <div className="rounded-2xl border border-neutral-200 bg-white p-6">
      <div className="flex items-center justify-between"><div><p className="text-[9px] font-bold uppercase tracking-widest text-neutral-500">Today's activity</p><p className="mt-1 text-[9px] text-neutral-500">{total} order{total === 1 ? '' : 's'} · peak {peak.hour.toString().padStart(2, '0')}:00</p></div><span className="grid h-9 w-9 place-items-center rounded-xl bg-neutral-900 text-white"><Activity size={16} /></span></div>
  ...
  return (
    <AdminLayout title="Dashboard">
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="font-sans text-[7px] font-semibold text-neutral-900">{greeting}, {auth?.user?.name?.split(' ')[0] || 'Admin'}</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-50 px-3 py-1.5 text-[9px] font-semibold text-emerald-700 ring-1 ring-emerald-200"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-500" />Live{lastSync ? ` · ${lastSync.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}` : ''}</span>
          <span className="hidden items-center gap-1.5 rounded-full bg-neutral-100 px-3 py-1.5 text-[9px] font-semibold text-neutral-600 sm:inline-flex"><Calendar size={12} /> {new Date().toLocaleDateString('en-US', { weekday: 'long', day: 'numeric', month: 'long' })}</span>
          <button onClick={() => load()} disabled={refreshing} className="inline-flex min-h-[34px] items-center gap-1.5 rounded-full border border-neutral-200 bg-white px-3 py-1.5 text-[9px] font-semibold text-neutral-700 transition hover:bg-neutral-50 disabled:opacity-50"><RefreshCw size={12} className={refreshing ? 'animate-spin' : ''} /> Refresh</button>
          <button onClick={() => exportDashboardSummary({ d, goal, alerts, insights: smart, storeName: 'HUSHAE', compareLabel: compare?.label || '' })} className="inline-flex min-h-[34px] items-center gap-1.5 rounded-full border border-neutral-200 bg-white px-3 py-1.5 text-[9px] font-semibold text-neutral-700 transition hover:bg-neutral-50"><Download size={12} /> Export</button>
      ...
      {d.kpis.profit && (d.kpis.profit.value !== 0 || d.kpis.cost.value !== 0) && (
        <div className="mb-6 rounded-2xl border border-neutral-200 bg-white p-6">
          <div className="mb-5 flex items-center justify-between"><div><p className="text-[9px] font-bold uppercase tracking-widest text-neutral-500">Profit & loss</p><p className="mt-1 text-[9px] text-neutral-500">Last 30 days · based on cost prices</p></div><Link to="/admin/products" className="text-[9px] font-semibold text-neutral-500 hover:text-neutral-900">Manage costs →</Link></div>
          ...
          {d.kpis.cost.value === 0 && <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-[9px] text-amber-800">💡 Set <b>Cost / Wholesale price</b> on each product for accurate profit tracking.</div>}
        </div>
      )}
      ...
      <div className="mb-6 grid gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-neutral-200 bg-white p-6">
          <div className="flex items-center justify-between"><div><p className="text-[9px] font-bold uppercase tracking-widest text-neutral-500">Best sellers</p><p className="mt-1 text-[9px] text-neutral-500">Top 5 by units sold</p></div><Sparkles size={16} className="text-amber-500" /></div>
          {d.bestSellers.length === 0 ? <p className="mt-6 text-center text-sm text-neutral-400">Sales data will appear here.</p> : (
            <ol className="mt-5 space-y-3">{d.bestSellers.map((b, i) => (
              <li key={b.name} className="flex items-center gap-3"><span className={`grid h-7 w-7 shrink-0 place-items-center rounded-lg text-[9px] font-bold ${i === 0 ? 'bg-amber-500 text-white' : i === 1 ? 'bg-neutral-400 text-white' : i === 2 ? 'bg-amber-700 text-white' : 'bg-neutral-100 text-neutral-600'}`}>{i + 1}</span>{b.image && <Img src={b.image} alt="" className="h-10 w-8 rounded-md border border-neutral-200 object-cover" />}<div className="min-w-0 flex-1"><p className="truncate text-[10px] font-medium text-neutral-900">{b.name}</p><p className="text-[9px] text-neutral-500">{b.qty} sold · {pkr(b.revenue)}</p></div></li>
            ))}</ol>
          )}
        </div>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="rounded-2xl border border-neutral-200 bg-white p-6 lg:col-span-2">
          <div className="mb-4 flex items-center justify-between"><div><p className="text-[9px] font-bold uppercase tracking-widest text-neutral-500">Recent orders</p><p className="mt-1 text-[9px] text-neutral-500">Last 6 across all stages</p></div><Link to="/admin/orders" className="inline-flex items-center gap-1 text-[9px] font-semibold text-neutral-500 hover:text-neutral-900">View all <ChevronRight size={12} /></Link></div>
          <div className="space-y-2">{d.recentOrders.map((o) => (
            <Link key={o._id} to={`/admin/orders/${o._id}`} className="flex items-center gap-3 rounded-xl border border-neutral-100 p-3 transition hover:border-neutral-300 hover:bg-neutral-50">
              ...
              <div className="min-w-0 flex-1"><div className="flex items-center gap-2"><p className="truncate font-mono text-[9px] font-semibold text-neutral-900">{o.orderNumber}</p><span className={`pill ${statusPill(o.status)}`}>{o.status}</span></div><p className="mt-0.5 truncate text-[9px] text-neutral-500">{o.customerInfo?.name} · {o.customerInfo?.city} · {fmtDate(o.createdAt)}</p></div>
              <div className="text-right"><p className="font-sans text-[8px] font-semibold tabular-nums text-neutral-900">{pkr(o.total)}</p><p className="text-[10px] text-neutral-400">{o.paymentMethod}</p></div>
            </Link>
          ))}</div>
        </div>
        <div className="space-y-6">
          <div className="rounded-2xl border border-neutral-200 bg-white p-6">
            <div className="mb-3 flex items-center justify-between"><p className="flex items-center gap-2 text-[9px] font-bold uppercase tracking-widest text-neutral-500"><AlertTriangle size={13} className="text-red-500" /> Low stock (≤ 10)</p><Link to="/admin/products" className="text-[9px] font-semibold text-neutral-500 hover:text-neutral-900">Manage</Link></div>
          ...
          <div className="rounded-2xl border border-neutral-200 bg-white p-6">
            <div className="mb-3 flex items-center justify-between"><p className="text-[9px] font-bold uppercase tracking-widest text-neutral-500">Top customers</p><Link to="/admin/customers" className="text-[9px] font-semibold text-neutral-500 hover:text-neutral-900">All</Link></div>
            {d.topCustomers.length === 0 ? <p className="py-6 text-center text-sm text-neutral-400">No customer data yet.</p> : <div className="space-y-3">{d.topCustomers.map((c, i) => (
              <div key={c.phone + i} className="flex items-center gap-3"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-neutral-900 text-[9px] font-semibold text-white">{(c.name || '?').slice(0, 1).toUpperCase()}</span><div className="min-w-0 flex-1"><p className="truncate text-[9px] font-semibold text-neutral-900">{c.name}</p><p className="truncate text-[10px] text-neutral-500">{c.city} · {c.orders} order{c.orders === 1 ? '' : 's'}</p></div><p className="font-sans text-[7px] font-semibold tabular-nums text-neutral-900">{pkr(c.spent)}</p></div>
            ))}</div>}
          </div>
        </div>
      </div>
