...
const NAV_ITEM_CLS = 'flex items-center gap-2 rounded-[7px] px-[10px] py-[7px] text-[12px] transition-colors';
const navItemStyle = (isActive) => ({
  fontWeight: isActive ? 600 : 400,
  background: isActive ? 'var(--fs-accent-soft-bg)' : 'transparent',
  color: isActive ? 'var(--fs-accent-soft-text)' : 'var(--fs-text-muted)',
});
const linkCls = ({ isActive }) => NAV_ITEM_CLS;
const linkStyle = ({ isActive }) => navItemStyle(isActive);

const CHILD_CLS = 'flex items-center gap-1.5 rounded-[7px] py-[6px] pl-7 pr-2 text-[12px] transition-colors';
const childLinkCls = () => CHILD_CLS;
  ...
  return (
    <div>
      <button type="button" onClick={() => setOpen((v) => !v)} aria-expanded={open}
        className={NAV_ITEM_CLS} style={navItemStyle(isChildActive)}>
      ...
      {open && <div className="mt-0.5 space-y-0.5">{group.children.map((c) => {
        ...
        return <NavLink key={c.to} to={c.to} onClick={onNavigate} className={childLinkCls} style={childLinkStyle}><ChildIcon size={12} strokeWidth={1.6} style={{ color: active ? 'var(--fs-accent-soft-text)' : 'var(--fs-text-muted)' }} />{c.label}</NavLink>;
      })}</div>}
    </div>
  );
}
...
function SidebarContent({ onNavigate }) {
  ...
  return (
    <div className="flex h-full flex-col" style={{ background: 'var(--fs-bg-sidebar)', borderRight: '1px solid var(--fs-border-subtle)' }}>
      <div className="px-4 pb-3 pt-5">
        <NavLink to="/admin" onClick={onNavigate} className="flex items-center gap-2.5">
          <span className="grid h-[22px] w-[22px] place-items-center rounded-[6px] text-[11px] font-bold text-white" style={{ background: 'linear-gradient(135deg, #6C5CE7, #5B4FE9)' }} aria-hidden="true">H</span>
          <span className="text-[13px] font-semibold" style={{ color: 'var(--fs-text-primary)' }}>Hushae</span>
        </NavLink>
      </div>
      {role && role !== 'admin' && role !== 'Owner' && (
        <div className="mx-4 mb-2">
          <p className="text-[11px] font-medium" style={{ color: 'var(--fs-text-muted)' }}>{getRoleLabel(role)} view</p>
        </div>
      )}
      <div className="relative mx-3 mb-2">
        <Search size={13} className="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2" style={{ color: 'var(--fs-text-muted)' }} />
        <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search…"
          className="w-full rounded-[8px] border bg-transparent py-1.5 pl-8 pr-7 text-[12px] outline-none"
          style={{ borderColor: 'var(--fs-border-subtle)', color: 'var(--fs-text-primary)' }} />
        <kbd className="pointer-events-none absolute right-2.5 top-1/2 -translate-y-1/2 rounded border px-1 py-0.5 text-[10px] font-medium" style={{ borderColor: 'var(--fs-border-medium)', color: 'var(--fs-text-muted)' }}><Command size={9} />K</kbd>
        {filtered.length > 0 && <div className="absolute inset-x-0 top-full z-10 mt-1 max-h-72 overflow-y-auto rounded-[10px] border py-1" style={{ background: 'var(--fs-bg-card)', borderColor: 'var(--fs-border-subtle)' }}>{filtered.map((f) => <Link key={f.to} to={f.to} onClick={() => { setQuery(''); onNavigate?.(); }} className="flex items-center gap-2 px-3 py-2 text-[12px] transition-colors hover:bg-white/5" style={{ color: 'var(--fs-text-secondary)' }}><f.icon size={13} style={{ color: 'var(--fs-text-muted)' }} /> {f.label}</Link>)}</div>}
      </div>
      <nav className="flex-1 space-y-0.5 overflow-y-auto px-2.5 pb-3">
        {NAV_TOP.map(({ to, label, icon: Icon, end }) => (
          <NavLink key={to} to={to} end={end} className={linkCls} style={linkStyle} onClick={onNavigate}>
            {({ isActive }) => <><Icon size={14} strokeWidth={1.6} style={{ color: isActive ? 'var(--fs-accent-soft-text)' : 'var(--fs-text-muted)' }} />{label}</>}
          </NavLink>
        ))}
        <div className="mt-2" />
        {visibleGroups.map((g) => <GroupDropdown key={g.label} group={g} onNavigate={onNavigate} defaultOpen={activeGroupLabel === g.label} />)}
      </nav>
      <div className="px-2.5 py-3" style={{ borderTop: '1px solid var(--fs-border-subtle)' }}>
        <button onClick={logout} className="flex w-full items-center gap-2.5 rounded-[7px] px-[10px] py-[7px] text-[12px] transition-colors hover:bg-white/5" style={{ color: 'var(--fs-text-muted)' }}><LogOut size={14} strokeWidth={1.6} /> Sign Out</button>
      </div>
    </div>
  );
}
...
export default function AdminLayout({ children, title }) {
  ...
  return (
    <div className="admin-shell flex min-h-screen" style={{ background: 'var(--fs-bg-page)', color: 'var(--fs-text-primary)' }}>
      ...
      <div className="flex min-h-screen min-w-0 flex-1 flex-col md:pl-[220px]">
        <div className="sticky top-0 z-30 flex items-center gap-3 border-b px-4 py-3 md:hidden" style={{ background: 'var(--fs-bg-sidebar)', borderColor: 'var(--fs-border-subtle)' }}><button onClick={() => setDrawer(true)} className="rounded-lg p-1.5" style={{ color: 'var(--fs-text-secondary)' }}><Menu size={20} /></button><Link to="/admin" className="text-[13px] font-semibold" style={{ color: 'var(--fs-text-primary)' }}>Hushae</Link></div>
        <TopBar title={title} auth={auth} onCmdK={() => setCmdOpen(true)} />
        <div className="min-w-0 flex-1" style={{ padding: '26px 30px' }}>{title && <h1 className="mb-6 text-[20px] font-bold leading-tight md:hidden" style={{ color: 'var(--fs-text-primary)', letterSpacing: '-0.3px' }}>{title}</h1>}<div className="admin-main min-w-0">{children}</div></div>
      </div>
      <ProfitCalculator />
...
function TopBar({ title, auth, onCmdK }) {
  ...
  return (
    <header className="sticky top-0 z-20 hidden border-b px-[30px] py-3 backdrop-blur md:block" style={{ borderColor: 'var(--fs-border-subtle)', background: 'rgba(10,10,12,0.85)' }}>
      <div className="flex items-center justify-between gap-6">
        <div className="min-w-0 flex-1">
          <h1 className="truncate text-[20px] font-bold leading-tight" style={{ color: 'var(--fs-text-primary)', letterSpacing: '-0.3px' }}>{title || crumbs[crumbs.length - 1]?.label}</h1>
          <nav className="mt-0.5 flex items-center gap-1.5 text-[12px]" style={{ color: 'var(--fs-text-muted)' }}>{crumbs.map((c, i) => <span key={c.to} className="inline-flex items-center gap-1.5">{i > 0 && <span className="opacity-40">/</span>}{i === crumbs.length - 1 ? <span style={{ color: 'var(--fs-text-secondary)' }}>{c.label}</span> : <Link to={c.to} className="transition-opacity hover:opacity-70">{c.label}</Link>}</span>)}</nav>
        </div>
        <div className="flex items-center gap-2">
          {/* ⌘K Search — ghost */}
          <button onClick={onCmdK} className="hidden items-center gap-1.5 rounded-[8px] px-2.5 py-2 text-[12px] font-medium transition-colors hover:bg-white/5 md:inline-flex" style={{ color: 'var(--fs-text-muted)' }} title="Search admin (⌘K)"><Command size={13} strokeWidth={1.5} /> Search</button>
          <span className="hidden items-center gap-1.5 text-[12px] lg:inline-flex" style={{ color: 'var(--fs-text-muted)' }}><span className="h-1.5 w-1.5 rounded-full" style={{ background: 'var(--fs-success)' }} aria-hidden="true" />Store online</span>
          {/* View store — secondary outline */}
          <Link to="/" target="_blank" className="hidden items-center gap-1.5 rounded-[8px] border px-[14px] py-2 text-[12px] font-medium transition-colors hover:bg-white/5 active:scale-[0.98] md:inline-flex" style={{ borderColor: 'var(--fs-border-medium)', color: 'var(--fs-text-secondary)' }} title="Open storefront"><Globe size={13} strokeWidth={1.5} /> View store</Link>
          {/* Create — secondary outline (dropdown) */}
          {canCreate && <div className="relative" ref={createRef}>
            <button onClick={() => setCreateOpen((v) => !v)} className="inline-flex items-center gap-1 rounded-[8px] border px-[14px] py-2 text-[12px] font-medium transition-colors hover:bg-white/5 active:scale-[0.98]" style={{ borderColor: 'var(--fs-border-medium)', color: 'var(--fs-text-secondary)' }}>Create <ChevronDown size={12} /></button>
            {createOpen && <div className="absolute right-0 top-full z-30 mt-1 w-56 rounded-[10px] border py-1" style={{ background: 'var(--fs-bg-card)', borderColor: 'var(--fs-border-subtle)' }}><Link to="/admin/promotions/new" onClick={() => setCreateOpen(false)} className="flex items-center gap-2.5 px-3.5 py-2 text-[13px] transition-colors hover:bg-white/5" style={{ color: 'var(--fs-text-secondary)' }}><Megaphone size={13} style={{ color: 'var(--fs-text-muted)' }} /> New promotion</Link><Link to="/admin/discounts" onClick={() => setCreateOpen(false)} className="flex items-center gap-2.5 px-3.5 py-2 text-[13px] transition-colors hover:bg-white/5" style={{ color: 'var(--fs-text-secondary)' }}><BadgePercent size={13} style={{ color: 'var(--fs-text-muted)' }} /> New discount</Link><Link to="/admin/cms/new" onClick={() => setCreateOpen(false)} className="flex items-center gap-2.5 px-3.5 py-2 text-[13px] transition-colors hover:bg-white/5" style={{ color: 'var(--fs-text-secondary)' }}><FileText size={13} style={{ color: 'var(--fs-text-muted)' }} /> New page</Link><Link to="/admin/blog/new" onClick={() => setCreateOpen(false)} className="flex items-center gap-2.5 px-3.5 py-2 text-[13px] transition-colors hover:bg-white/5" style={{ color: 'var(--fs-text-secondary)' }}><FileText size={13} style={{ color: 'var(--fs-text-muted)' }} /> New blog article</Link></div>}
          </div>}
          {/* + Add product — the ONE primary (solid accent) */}
          {canCreate && <Link to="/admin/products/new" className="inline-flex items-center gap-1.5 rounded-[8px] px-[14px] py-2 text-[12px] font-semibold text-[#FFFFFF] transition-colors hover:bg-[var(--fs-accent-hover)] active:scale-[0.98]" style={{ background: 'var(--fs-accent)' }}><Plus size={13} /> Add product</Link>}
          {/* Theme toggle — icon, ghost */}
          <button onClick={toggleDark} aria-label={dark ? 'Switch to light mode' : 'Switch to dark mode'} title={dark ? 'Switch to light mode' : 'Switch to dark mode'} className="hidden items-center justify-center rounded-[8px] p-2 transition-colors hover:bg-white/5 md:inline-flex" style={{ color: 'var(--fs-text-muted)' }}>
            {dark ? <Sun size={14} /> : <Moon size={14} />}
          </button>
          <NotificationBell />
          <div className="ml-1 flex items-center gap-2.5"><span className="grid h-7 w-7 place-items-center rounded-full text-[12px] font-semibold text-[#FFFFFF]" style={{ background: 'var(--fs-accent)' }}>{initials}</span><span className="text-[13px]" style={{ color: 'var(--fs-text-secondary)' }}>{auth?.user?.name?.split(' ')[0] || 'Admin'}</span></div>
        </div>
      </div>
    </header>
  );
