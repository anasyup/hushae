...
- **2026-08-16** — 🏁 **COMPLETION BATCH — the remaining 8 dashboard/ops features**
  ...
  - **F8 dark mode (opt-in)**: restored the `.dark-admin` system + `admin-dark.css` and re-added the sun/moon toggle in the top bar next to the bell. Defaults to LIGHT on every device (honouring the earlier "dark theme khatam karo" removal) — dark is now an explicit per-device choice, stored in localStorage.
  - **F9/F11 verification + call queue**: new `/admin/verification-queue` page (Orders → Verification queue) listing all orders awaiting payment verification 24h+ (oldest first) with WhatsApp deep-link, tap-to-call, reliability badge, and Verified / No Answer / Cancel buttons (cancel requires a reason). Keyboard shortcuts V/N/C, auto-advance, and no-answer attempts tracked server-side (`Order.noAnswer`) with 3+ auto-flagging. Backend: `GET /orders/manage/verification-queue`, `PATCH /orders/manage/:id/verify-action`. The "pending payment verification" alert now links straight to the queue.
  - **F10 reliability score**: server-side `utils/customerReliability.js` (keyed by phone): Reliable / New customer / High risk with cancel-rate maths. Badges on the Orders list (OrderRow), Order Detail, Top Customers and the verification queue. 19 assertions in `tests/customer-reliability.mjs`.
  - **F12 cancellation reasons**: required reason dropdown (6 options + Other) when staff cancel — in the orders desk, order detail, and the call queue; captured on `Order.cancelReason`. New dashboard widget "Cancellation reasons" (ranked bars + top-reason callout) + `GET /orders/manage/cancellation-reasons`; the "cancelled this week" alert now links to it.
  - **F13 low-stock reorder**: one-click Reorder on every low-stock row → modal with current/target/suggested quantity, shareable WhatsApp summary for the supplier, "mark reorder placed" flag (`Product.reorderStatus`) and "mark received" with optional stock update. Settings → `reorderTargetStock` (default 50).
  - **F14 abandoned carts**: dashboard widget (open count + potential revenue + recent carts with one-click WhatsApp nudge) wired to the existing `POST /abandoned-cart/track` storefront capture and `GET /abandoned-cart/admin`.
  - Tests: 8/8 suites passing (added customer-reliability). Smoke-tested the new aggregations against the production Atlas DB (read-only): verification queue correctly surfaces the 11 pending orders.

- **2026-08-16** — 📈 **DASHBOARD polish batch — growth %, pipeline, quick actions, charts, contrast, WhatsApp verify**
  - **Growth % fix**: new shared `growthPct(current, previous)` in `utils/helpers.js` replaces the buggy `!b ? (a > 0 ? 100 : 0) : …` in BOTH `admin.js` (KPI cards) and `dashboardSignals.js` (/dashboard/compare). When the previous period is 0 it now returns `null` — the KPI cards show a neutral "New" chip (current > 0) or nothing (current = 0) instead of a fake green ▲100%. Normal cases keep 1-decimal rounding; previous>0 & current=0 → -100%. Unit-tested in `tests/growth-pct.mjs` (13 assertions).
  - **Quick actions**: exactly one primary — "Add product" (filled black); View orders / New promo / Discounts are equal-weight outline buttons with subtle hover/active tints.
  - **Order pipeline**: the bar is now 6 FIXED equal segments (~16.6% each) instead of proportional widths. Segments with orders are stage-tinted; empty stages render as light neutral placeholders (never collapsed). Hovering a segment shows a tooltip with the exact count.
  - **Chart empty states**: "Today's activity" and "Peak order hours" show "No orders yet today" / "No orders in this period" instead of a flat all-zero chart. Added a per-widget `ChartBoundary` (retry card) around Revenue chart, Status donut, Today activity and Peak hours.
  - **WCAG AA contrast**: programmatic audit found `neutral-400` (#AAAAAA) at 2.32:1 on white. Darkened centrally to #6B7280 (4.83:1 on white). The dark "Smart insight" card now uses `text-gray-400` (#9CA3AF, 7.44:1 on #111) since a single gray cannot pass on both surfaces.
  - **WhatsApp verify**: every Pending order in the Recent Orders widget now has a one-click WhatsApp button (wa.me deep link, no API key) pre-filling "Hi {name}, confirming order {orderNumber} for PKR {total}…". Store phone pulled from settings.
...
- **2026-08-11** — 🎨 **CARD → Givenchy studio canvas style**: image box now `p-6` with `object-contain mix-blend-multiply` (studio canvas look), secondary image crossfades on hover (no zoom), details centered (title 11/12px medium uppercase tracking 0.15em, price centered). Kept arrows, hover-swap, Buy Now pill + SizeModal per earlier requests.
- **2026-08-11** — 🧹 **LISTING PAGE — clean reference**: removed duplicate sub-category bar + old FilterPills; now a SINGLE LuxuryFilterBar (pills + item count + Sort By) above the product grid on all listing pages, exactly per the "Clean Listing Page" reference.
- **2026-08-11** — 🧹 **SALE PAGE — single filter bar**: removed the filter-chips bar inside SalePageHeader (title + gender tabs kept); the single LuxuryFilterBar now serves the sale page too — no duplicate filter rows.
- **2026-08-11** — 🎬 **CARD + HEADER — exact references**: (1) card → layered crossfade images (z-index/opacity), 4/3 → sm:3/4, side arrows on hover, full-width BUY NOW button (w-85%, opens SizeModal), centered details. (2) header → transparent over hero, turns white/95 + blur + shadow on scroll (>20px), logo/nav/icons white→black transition, cart badge inverts.
- **2026-08-11** — 🧭 **HEADER — exact final reference**: scroll threshold 30px, announcement bar ALWAYS visible (black, 10/11px, tracking 0.2em, leading-none, py-2.5, bold underlined SHOP), header border-none, scrolled py-3 / top py-4, logo 2xl/3xl tracking 0.2em, chevron stroke-2, overflow-x hidden already global.
- **2026-08-11** — 🏷️ **CARD DETAILS — exact reference**: title → sentence case 14px medium tracking -0.01em (#1a1a1a), color swatches row (13px, ring selection, switches gallery image), price 13px medium + struck original. Kept arrows + full-width BUY NOW + hover crossfade.
- **2026-08-11** — 🧹 **HEADER final polish**: icons stroke 1.5→1.8, cart badge font-bold, global reset `html, body, #root { margin/padding 0 !important; top 0; overflow-x hidden }` added (tokens.css) so the fixed header sits flush at the very top with zero browser spacing.
- **2026-08-11** — 🧹 **HEADER flush fix**: added `!m-0 !p-0` on the fixed header wrapper and `!m-0` on the announcement bar (important zero spacing, per reference Option 3). Global reset (`html, body, #root` margin/padding/top 0) already present.
- **2026-08-11** — 🏷️ **CARD TITLE → button/link**: product title below the card is now a clickable link to the product page (hover = grey + underline). Everything else untouched per user request.
