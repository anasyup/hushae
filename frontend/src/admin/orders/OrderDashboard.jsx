  ...
  return (
    <div className="space-y-3 rounded-xl border border-neutral-200 bg-white p-4">
      ...
      <div className="flex flex-wrap items-center gap-x-5 gap-y-2 rounded-lg bg-neutral-50 px-3 py-2.5 text-[12px]">
        ...
        <span className="flex items-center gap-1.5">
          ...
          {Object.entries(data.byMethod).map(([m, n]) => (
            <span key={m} className="rounded-full bg-white px-2 py-0.5 text-[12px] font-medium text-neutral-700 ring-1 ring-neutral-200">
              {m} {n}
            </span>
          ))}
        </span>
      ...
      <div className="rounded-lg border border-neutral-200 p-3">
        <p className="mb-2 text-[13px] font-semibold uppercase tracking-wider text-neutral-500">Pipeline</p>
            ...
            return (
              <button key={p.group} onClick={() => onPipelineClick?.(p.group)}
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="truncate text-[12px] font-medium text-neutral-600">{g?.label || p.group}</span>
                </div>
                <p className="mt-0.5 text-[13px] font-semibold tabular-nums leading-none text-neutral-900">{p.count}</p>
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        ...
        <Panel title="Payment mix">
          ...
          <div className="flex flex-wrap justify-center gap-x-3 gap-y-1">
            {methodData.map((m, i) => (
              <span key={m.name} className="inline-flex items-center gap-1 text-[12px] text-neutral-600">
            ...
            <ul className="space-y-1.5">
              {data.topProducts.map((p, i) => (
                <li key={p.name} className="flex items-center gap-2 text-[12px]">
                  <span className="w-4 text-[12px] font-bold text-neutral-300">{i + 1}</span>
          ...
          {data.topCities.length === 0 ? <Empty /> : (
            <ul className="space-y-1.5">
              {data.topCities.map((c) => (
                <li key={c.city} className="flex items-center gap-2 text-[12px]">
                  <span className="min-w-0 flex-1 truncate text-neutral-700">{c.city}</span>
                  <span className="shrink-0 text-[12px] text-neutral-400">{pkr(c.revenue)}</span>
...
function Panel({ title, children }) {
  return (
    <div className="rounded-lg border border-neutral-200 p-3">
      <p className="mb-2 text-[13px] font-semibold uppercase tracking-wider text-neutral-500">{title}</p>
      {children}
    </div>
  );
}
