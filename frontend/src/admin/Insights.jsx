  ...
  return (
    <AdminLayout title="Insights">
      ...
      <section className="mb-10">
        ...
        <div className="adm-divide-x grid grid-cols-1 border-y border-white/10 sm:grid-cols-3">
          <div className="px-5 py-6">
            ...
            <p className="mt-2 text-[11px] text-white/30">{peakHour.orders} orders</p>
          </div>
          <div className="px-5 py-6">
            <p className="adm-label">Repeat rate</p>
            <p className="adm-metric mt-3 text-[32px] leading-none text-white">{d.repeat.rate}%</p>
            <p className="mt-2 text-[11px] text-white/30">{d.repeat.repeat} of {d.repeat.total} buyers came back</p>
          </div>
          <div className="px-5 py-6">
      ...
      <section className="mb-10">
        ...
        <p className="mb-4 text-[12px] text-white/35">Distribution of orders across the day — plan promotions at the peak.</p>
        <div className="h-56">
      ...
      <div className="mb-10 grid gap-10 lg:grid-cols-2">
        <section>
          ...
          <p className="mb-4 text-[12px] text-white/35">By revenue — focus your ads here.</p>
          {d.topCities.length === 0 ? (
            <p className="border-y border-white/10 py-8 text-center text-[12px] text-white/35">No delivery data yet.</p>
          ) : (
            <RankedBars
        ...
        <section>
          ...
          <p className="mb-4 text-[12px] text-white/35">Top 10 by gross profit (needs cost prices set).</p>
          {d.topProfit.length === 0 || d.topProfit.every((p) => p.profit === 0) ? (
            <p className="border-y border-white/10 py-8 text-[12px] leading-relaxed text-white/35">
              Set the Cost / Wholesale price field on each product to see accurate profit ranking here.
            </p>
          ) : (
            <ol>
              {d.topProfit.map((p, i) => (
                <li key={p._id} className="flex items-center gap-3 border-b border-white/5 py-2.5">
                  <span className="w-6 shrink-0 text-[10px] uppercase tracking-[0.16em] text-white/30">{String(i + 1).padStart(2, '0')}</span>
                  {p.image && <Img src={p.image} alt="" className="h-10 w-8 shrink-0 border border-white/10 object-cover" />}
                  <div className="min-w-0 flex-1">
                    <p className="line-clamp-1 text-[12px] text-white">{p.name}</p>
                    <p className="text-[11px] text-white/30">{p.unitsSold} sold · {pkr(p.revenue)}</p>
                  </div>
                  <div className="text-right">
                    <p className="adm-metric text-[13px] text-white">{pkr(p.profit)}</p>
                    <p className="text-[10px] uppercase tracking-[0.12em] text-white/30">profit</p>
                  </div>
                </li>
              ))}
            </ol>
      ...
      <section>
        ...
        <p className="mb-4 text-[12px] text-white/35">Customers grouped by the month of their first order.</p>
        {d.cohort.length === 0 ? (
          <p className="border-y border-white/10 py-8 text-center text-[12px] text-white/35">Need more historical orders to build cohorts.</p>
        ) : (
                ...
                {d.cohort.map((c) => {
                  ...
                  return (
                    <tr key={c._id} className="border-b border-white/5">
                      <td className="py-2.5 font-mono text-[12px] text-white/80">{c._id}</td>
                      <td className="py-2.5 text-right tabular-nums text-white/70">{c.newCustomers}</td>
                      <td className="py-2.5 text-right tabular-nums text-white/70">{c.repeatCustomers}</td>
