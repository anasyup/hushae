...
export function Num({ label, hint, value, onChange, disabled, ...rest }) {
  const id = useId();
  return (
    <div>
...
export function Text({ label, hint, value, onChange, disabled, ...rest }) {
  const id = useId();
  return (
    <div>
      <label className="mb-1 block text-[10px] font-bold uppercase tracking-wider text-neutral-500" htmlFor={id}>{label}</label>
...
export function Select({ label, hint, value, onChange, options, disabled }) {
  const id = useId();
  return (
    <div>
      <label className="mb-1 block text-[10px] font-bold uppercase tracking-wider text-neutral-500" htmlFor={id}>{label}</label>
...
export function DateTime({ label, hint, value, onChange, disabled }) {
  const id = useId();
