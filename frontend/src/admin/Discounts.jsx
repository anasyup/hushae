    ...
    <AdminLayout title="Discounts">
      <div className="mb-6 flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-start gap-4">
          ...
          <div><h2 className="font-sans text-2xl text-neutral-900">Discounts</h2><p className="mt-1 text-[13px] text-neutral-600">Coupon codes customers enter at checkout. Track usage per code.</p></div>
        </div>
      ...
      {showForm && (
        <form onSubmit={save} className="mb-5 overflow-hidden rounded-2xl border border-neutral-200 bg-white">
          <div className="flex items-center justify-between border-b border-neutral-100 px-6 py-4"><h3 className="font-sans text-[16px] font-semibold text-neutral-900">{editing ? `Edit ${form.code}` : 'New discount code'}</h3><button type="button" onClick={() => setShowForm(false)} className="rounded-full p-2 text-neutral-400 hover:bg-neutral-100 hover:text-neutral-900"><X size={16} /></button></div>
          <div className="grid gap-4 p-6 sm:grid-cols-2 lg:grid-cols-3">
            <div><label className="mb-1 block text-[11px] font-bold uppercase tracking-widest text-neutral-500">Code</label><input className="w-full rounded-xl border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900 uppercase" placeholder="WELCOME10" value={form.code} onChange={(e) => set('code', e.target.value)} required /></div>
            <div><label className="mb-1 block text-[11px] font-bold uppercase tracking-widest text-neutral-500">Type</label><select className="w-full rounded-xl border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900" value={form.type} onChange={(e) => set('type', e.target.value)}><option value="percent">Percentage (%)</option><option value="fixed">Fixed amount (PKR)</option></select></div>
            <div><label className="mb-1 block text-[11px] font-bold uppercase tracking-widest text-neutral-500">{form.type === 'percent' ? 'Percent off' : 'Amount off (PKR)'}</label><input className="w-full rounded-xl border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900" type="number" min="1" max={form.type === 'percent' ? 100 : undefined} value={form.value} onChange={(e) => set('value', e.target.value)} required /></div>
            <div><label className="mb-1 block text-[11px] font-bold uppercase tracking-widest text-neutral-500">Min order (optional)</label><input className="w-full rounded-xl border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900" type="number" min="0" placeholder="0" value={form.minSubtotal} onChange={(e) => set('minSubtotal', e.target.value)} /></div>
            <div><label className="mb-1 block text-[11px] font-bold uppercase tracking-widest text-neutral-500">Max uses (0 = unlimited)</label><input className="w-full rounded-xl border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900" type="number" min="0" placeholder="0" value={form.maxUses} onChange={(e) => set('maxUses', e.target.value)} /></div>
            <div><label className="mb-1 block text-[11px] font-bold uppercase tracking-widest text-neutral-500">Expiry date</label><input className="w-full rounded-xl border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900" type="date" value={form.expiresAt} onChange={(e) => set('expiresAt', e.target.value)} /></div>
          </div>
      ...
      {list && list.length > 0 && <div className="mb-4"><div className="relative max-w-sm"><Search size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-neutral-400" /><input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search codes…" className="w-full rounded-xl border border-neutral-300 bg-white px-3 py-2 text-[12px] outline-none transition focus:border-neutral-900 !w-full !py-2.5 !pl-9 !text-[13px]" /></div></div>}
      {!list ? <div className="animate-pulse rounded-xl bg-neutral-100 h-40 rounded-2xl" /> : list.length === 0 ? <div className="grid place-items-center rounded-2xl border border-dashed border-neutral-200 bg-white py-16 text-center"><BadgePercent size={32} className="mb-3 text-neutral-300" /><p className="text-[14px] font-medium text-neutral-700">No discount codes yet</p><p className="mt-1 text-[12px] text-neutral-500">Create your first code, e.g. WELCOME10.</p><button onClick={openNew} className="mt-4 inline-flex items-center gap-1.5 rounded-full bg-neutral-900 px-4 py-2 text-[12px] font-semibold text-white hover:bg-black"><Plus size={13} /> Create code</button></div> : filtered.length === 0 ? <div className="grid place-items-center rounded-2xl border border-dashed border-neutral-200 bg-white py-16 text-center"><Search size={28} className="mb-3 text-neutral-300" /><p className="text-[14px] font-medium text-neutral-700">No codes match "{q}"</p></div> : (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((d) => (
            <div key={d._id} className={`group rounded-2xl border bg-white p-5 transition hover:shadow-sm ${d.active ? 'border-neutral-200' : 'border-neutral-200 bg-neutral-50/50'}`}>
              ...
              <div className="mt-4">
                <div className="flex items-center justify-between text-[11px]"><span className="font-medium text-neutral-600">{d.usedCount || 0} use{(d.usedCount || 0) === 1 ? '' : 's'}</span>{d.maxUses > 0 && <span className="text-neutral-500">/{d.maxUses} max</span>}</div>
              ...
              <div className="mt-4 flex items-center gap-1 border-t border-neutral-100 pt-3">
                <button onClick={() => openEdit(d)} className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[11px] font-semibold text-neutral-600 transition hover:bg-neutral-100"><Plus size={11} className="rotate-45" /> Edit</button>
                <button onClick={() => remove(d)} className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[11px] font-semibold text-neutral-600 transition hover:bg-red-50 hover:text-red-600"><X size={11} /> Delete</button>
