 ...
 * renders 2x2 because all four on one row is too thin to read; on lg they
 * spread to one row. Zero images, so the section adds nothing to the
    ...
    >
      <div className="mx-auto grid max-w-[1600px] grid-cols-2 gap-y-4 px-5 py-5 md:grid-cols-4 md:gap-y-0 md:px-8 md:py-6">
        {PROMISES.map(({ icon: Icon, label }) => (
          <div
            key={label}
            className="flex items-center justify-center gap-3 text-[11px] font-medium uppercase tracking-[0.18em] text-black md:justify-start md:gap-3 md:text-[11.5px] md:tracking-[0.2em]"
          >
            <Icon
              size={18}
              strokeWidth={1.4}
