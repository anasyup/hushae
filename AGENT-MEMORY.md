...
- **IA (404 kabhi nahi, one home per concept):** Inventory → redirect `/admin/ops/inventory` · Bundles → redirect `/admin/bundles` · Import/Export → `/admin/products?import=1` (CSV modal auto-open) · Attributes/Digital/Product Settings → `admin/AdminReserved.jsx` honest pane. Nav label **Products** (was Catalog).
...
- **Phase 2 candidates:** Collections ko bhi ATELIER · Customers · Product Settings editor · global variant option templates · server-side pagination (>2k).
