...
async function resolveProducts(c, limit = 60) {
  ...
  if (Array.isArray(c.products) && c.products.length) {
    ...
    }).select('name slug price compareAtPrice onSale saleStart saleEnd stock images gender categorySlug tier fabric ratingAvg sizes colors isNewArrival isBestSeller tags');
