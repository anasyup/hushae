...
const EvIcon = ({ e }) =>
  e === 'cart' ? <ShoppingCart size={13} className="text-amber-700" />
    : e === 'checkout' ? <CreditCard size={13} className="text-sagedeep" />
    : <Globe size={13} className="text-ash" />;
...
export default function LiveView() {
  ...
  if (!d) return <AdminLayout title="Live View"><div className="skeleton h-64 w-full" /></AdminLayout>;
  ...
  return (
    <AdminLayout title="Live View">
      <div className="mb-5 flex items-center gap-2 text-xs text-ash">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-3">
        ...
        <div className="card p-6">
          <p className="mb-5 flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest text-ash"><Activity size={14} /> Customer behaviour — today</p>
              ...
              <div className="space-y-2.5">
                {d.byLocation.map((l) => (
                  <div key={`${l.city}-${l.country}`} className="flex items-center gap-3">
                    <span className="w-40 truncate text-xs font-medium">{l.city} <span className="text-ash">({l.country})</span></span>
                    <div className="h-2.5 flex-1 overflow-hidden rounded-full bg-satin">
                      <div className="h-full rounded-full bg-sagedeep" style={{ width: `${(l.sessions / maxLoc) * 100}%` }} />
                    </div>
      ...
      <div className="card mt-6 p-6">
        <p className="mb-4 text-[11px] font-bold uppercase tracking-widest text-ash">Live activity feed</p>
        {d.feed.length === 0 ? <p className="text-sm text-ash">Abhi koi activity nahi — store khulte hi yahan real-time feed chalegi.</p> : (
          <ol className="space-y-2">
            {d.feed.map((e, i) => (
              <li key={i} className="flex items-center gap-3 border-b border-line/50 pb-2 text-sm last:border-0 last:pb-0">
                <EvIcon e={e.event} />
                <span className="font-medium">{EV_LABEL[e.event]}</span>
                <span className="max-w-56 truncate font-mono text-xs text-ash">{e.path}</span>
                <span className="ml-auto flex items-center gap-2 text-[11px] text-ash">
