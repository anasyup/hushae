...
/* ============================================================================
 * HUSHAE Shop — CK Malaysia PLP style.
 * Clean, edge-to-edge, no masthead, minimal toolbar.
 * ========================================================================== */
...
export default function Shop({ preset = {} }) {
  ...
  return (
    <div style={{ fontFamily: "'Archivo', system-ui, sans-serif", background: '#FFFFFF', minHeight: '100vh' }}>
      <Seo
        title={`${meta}${f.gender ? ' — ' + f.gender.charAt(0).toUpperCase() + f.gender.slice(1) : ''}`}
        description={`Shop premium ${meta.toLowerCase()} — bras, briefs, shapewear and more. Made in Pakistan.`}
        canonical={typeof window !== 'undefined' ? window.location.pathname : '/shop'}
      />

      <div className="px-4 md:px-8 lg:px-12">

        {/* ═══ CK HEADER — just a category title, minimal ═══════════ */}
        <div className="flex items-baseline justify-between pt-8 pb-2">
          ...
          {activeFilterCount > 0 && (
            <button onClick={f.clearAll} className="flex items-center gap-1 text-[11px] font-medium uppercase tracking-[0.10em] text-[#6E6E6B] hover:text-[#0E0E0E]">
              Clear all <X size={12} />
            </button>
          )}
        </div>

        {/* ═══ CK TOOLBAR — Filter+Sort row ════════════════════════ */}
        <div className="flex items-center justify-between border-b border-[#E3E2DF] pb-3 mb-6">
          <button
            ref={filterBtnRef}
            onClick={() => setSheetOpen(true)}
            className="inline-flex items-center gap-2 text-[12px] font-medium uppercase tracking-[0.10em] text-[#0E0E0E] hover:opacity-60">
            <SlidersHorizontal size={14} /> Filter and Sort
            {activeFilterCount > 0 && (
              <span className="inline-flex h-5 min-w-5 items-center justify-center rounded-full bg-[#0E0E0E] px-1.5 text-[10px] font-bold text-white">{activeFilterCount}</span>
            )}
          </button>

          <div className="flex items-center gap-4">
            {/* View selector — CK style */}
            <div className="hidden md:flex items-center gap-1">
              {[2, 3, 4].map((v) => (
                <button key={v} onClick={() => setView(v)}
                  className={`grid h-8 w-8 place-items-center text-[11px] font-medium ${view === v ? 'text-[#0E0E0E] underline underline-offset-4' : 'text-[#6E6E6B] hover:text-[#0E0E0E]'}`}>
                  {v}
                </button>
              ))}
            </div>
            <div className="relative">
              ...
              <ChevronDown size={12} className="pointer-events-none absolute right-0 top-1/2 -translate-y-1/2 text-[#6E6E6B]" />
            </div>
          </div>
        </div>
        ...
        {/* ═══ PRODUCT GRID — CK edge-to-edge ═══════════════════ */}
        {products === null ? (
          <ProductGridSkeleton />
        ) : count === 0 ? (
          <div>
            <EmptyState icon={SearchX} title="Nothing matches those filters"
              description="Try removing one — or clear them all and start again."
              onAction={f.clearAll} actionLabel="Clear all filters" />
            <ActiveChips chips={f.chips} onRemove={f.removeChip} onClearAll={f.clearAll} className="justify-center mt-4" />
          </div>
        ) : (
          <div aria-busy={pending || undefined}
            className={`grid grid-cols-2 gap-px md:gap-[2px] transition-opacity duration-300 ${
              view === 2 ? 'md:grid-cols-2' : view === 3 ? 'md:grid-cols-3' : 'md:grid-cols-4'
      ...
      <FilterSheet open={sheetOpen} onClose={() => setSheetOpen(false)} onReset={f.clearAll}
        ...
        f={f} resultCount={count} returnFocusTo={filterBtnRef} />
    </div>
  );
}
