...
function CollectionCard({ product: p, priority = false, variant = 'bar', ratio = 'aspect-[3/4.5]' }) {
  ...
  if (pill) {
    return (
      <article className="group relative flex flex-col">
        <Link to={`/product/${p.slug}`} tabIndex={-1} className="relative block w-full overflow-hidden bg-[#f3ede2]" style={{ aspectRatio: '3 / 4.5' }}>
          <img
  ...
  return (
    ...
    >
      ...
      <Link to={`/product/${p.slug}`} tabIndex={-1} className="relative block aspect-[3/4.5] w-full overflow-hidden bg-[#f4f4f2]">
