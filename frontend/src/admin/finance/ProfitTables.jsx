    ...
    <div className="rounded-2xl border border-neutral-200 bg-white p-5 sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="flex items-center gap-2 text-[9px] font-bold uppercase tracking-widest text-neutral-500">
            <Boxes size={13} className="text-neutral-400" /> Profit by product
          </p>
          <p className="mt-1 text-[9px] text-neutral-500">Sort by margin to find high-volume, low-margin lines that need a price review.</p>
        </div>
    ...
    <div className="rounded-2xl border border-neutral-200 bg-white p-5 sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 text-[9px] text-neutral-500">Flagged customers spend well but leave little behind — usually cancellations or returns.</p>
        </div>
        <select value={sort} onChange={(e) => setSort(e.target.value)}
          className="min-h-[34px] rounded-full border border-neutral-200 bg-white px-3 text-[9px] font-semibold text-neutral-700 outline-none focus:border-neutral-900">
          <option value="profit-desc">Most profit</option>
                ...
                <td className="py-2.5">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="truncate text-[9px] text-neutral-500">{r.city}</p>
                    </div>
                  </div>
                </td>
...
export function CodExposure() {
  ...
  return (
    <div className="rounded-2xl border border-neutral-200 bg-white p-5 sm:p-6">
      <p className="text-[9px] font-bold uppercase tracking-widest text-neutral-500">COD exposure</p>
      <p className="mt-1 text-[9px] text-neutral-500">Money you have not been paid yet, because the parcel has not been delivered.</p>
      ...
      <div className="mt-4 grid grid-cols-2 gap-2">
        <div className="rounded-xl bg-neutral-50 p-3">
          ...
          <p className="mt-0.5 text-[9px] text-neutral-500">Cancellable at no cost</p>
        </div>
        <div className="rounded-xl bg-amber-50 p-3">
          ...
          <p className="mt-0.5 text-[9px] text-amber-700">{pkr(d.sunkCost)} already spent</p>
        </div>
      </div>

      {d.oldestDays > 5 && (
        <p className="mt-3 rounded-lg bg-red-50 px-3 py-2 text-[10px] leading-relaxed text-red-800">
        ...
        <div className="mt-4 border-t border-neutral-100 pt-3">
          ...
          <ul className="mt-2 space-y-1.5">
            {d.risky.slice(0, 4).map((r) => (
              <li key={r.phone} className="flex items-center justify-between gap-2 text-[10px]">
...
export function BreakEven({ days, from, to }) {
  ...
  return (
    <div className="rounded-2xl border border-neutral-200 bg-white p-5 sm:p-6">
      <p className="text-[9px] font-bold uppercase tracking-widest text-neutral-500">Break-even</p>
      <p className="mt-1 text-[9px] text-neutral-500">How many orders a day cover your fixed monthly costs.</p>

      {unset ? (
        <p className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-[10px] leading-relaxed text-amber-800">
        ...
        <p className="mt-4 rounded-xl border border-red-200 bg-red-50 p-3 text-[10px] leading-relaxed text-red-800">
        ...
        <>
          <div className="mt-4 flex items-baseline gap-2">
            ...
            <p className="text-[9px] text-neutral-500">order{d.ordersNeededPerDay === 1 ? '' : 's'} / day needed</p>
          </div>

          <div className="mt-3 flex items-center gap-2">
            <span className={`rounded-full px-2.5 py-1 text-[9px] font-bold ring-1 ${d.onTrack ? 'bg-emerald-50 text-emerald-700 ring-emerald-200' : 'bg-amber-50 text-amber-800 ring-amber-200'}`}>
              {d.onTrack ? 'Covering costs' : 'Below break-even'}
            </span>
            <span className="text-[10px] text-neutral-500">running at {d.currentPerDay}/day</span>
          </div>

          <dl className="mt-4 space-y-1.5 border-t border-neutral-100 pt-3 text-[10px]">
            {[
