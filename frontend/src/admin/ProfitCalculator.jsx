          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-neutral-200 bg-neutral-900 px-5 py-3 text-white">
              <div className="flex items-center gap-2">
                <Calculator size={16} />
                <div>
                  <p className="text-[13px] font-bold uppercase tracking-widest text-neutral-400">Quick tool</p>
                  <p className="text-[12px] font-semibold">Profit Calculator</p>
                </div>
              </div>
              <div className="flex items-center gap-1">
                <button onClick={reset} className="rounded-full px-2.5 py-1 text-[12px] font-semibold text-neutral-300 transition hover:bg-white/10 hover:text-white">Reset</button>
            ...
            <div className="max-h-[70vh] overflow-y-auto p-5">
              ...
              <div className="mt-5 rounded-xl border border-neutral-200 bg-neutral-50 p-3 text-[12px] leading-relaxed text-neutral-600">
...
function Section({ title, children }) {
  return (
    <div>
      <p className="mb-1.5 text-[13px] font-bold uppercase tracking-widest text-neutral-500">{title}</p>
      {children}
    </div>
  );
}
...
function Field({ label, v, onChange, placeholder, prefix, suffix }) {
  return (
    <label className="block">
      <span className="mb-1 block text-[12px] font-semibold text-neutral-500">{label}</span>
      <span className="relative block">
        {prefix && <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[13px] font-bold text-neutral-400">{prefix}</span>}
        <input
          type="number"
          inputMode="decimal"
          value={v}
        ...
        />
        {suffix && <span className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[12px] font-bold text-neutral-400">{suffix}</span>}
      </span>
    </label>
  );
}
...
function OutputTile({ label, value, sub, tone = 'neutral', big, bigSub, small }) {
  ...
  return (
    <div className={`rounded-xl border p-3 ${map[tone]}`}>
      ...
      {sub && <p className={`mt-1 ${bigSub ? 'text-[12px] font-semibold' : 'text-[13px] opacity-70'}`}>{sub}</p>}
    </div>
  );
}
