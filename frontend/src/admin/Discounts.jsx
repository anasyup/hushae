...
export default function Discounts() {
  ...
  return (
    <AdminLayout title="Discounts">
      ...
      {showForm && (
        <form onSubmit={save} className="mb-5 overflow-hidden rounded-2xl border border-neutral-200 bg-white">
          ...
          <div className="grid gap-4 p-6 sm:grid-cols-2 lg:grid-cols-3">
            <div><label className="mb-1 block text-[11px] font-bold uppercase tracking-widest text-neutral-500">Code</label><input className="input uppercase" placeholder="WELCOME10" value={form.code} onChange={(e) => set('code', e.target.value)} required /></div>
            <div><label className="mb-1 block text-[11px] font-bold uppercase tracking-widest text-neutral-500">Type</label><select className="input" value={form.type} onChange={(e) => set('type', e.target.value)}><option value="percent">Percentage (%)</option><option value="fixed">Fixed amount (PKR)</option></select></div>
            <div><label className="mb-1 block text-[11px] font-bold uppercase tracking-widest text-neutral-500">{form.type === 'percent' ? 'Percent off' : 'Amount off (PKR)'}</label><input className="input" type="number" min="1" max={form.type === 'percent' ? 100 : undefined} value={form.value} onChange={(e) => set('value', e.target.value)} required /></div>
            <div><label className="mb-1 block text-[11px] font-bold uppercase tracking-widest text-neutral-500">Min order (optional)</label><input className="input" type="number" min="0" placeholder="0" value={form.minSubtotal} onChange={(e) => set('minSubtotal', e.target.value)} /></div>
            <div><label className="mb-1 block text-[11px] font-bold uppercase tracking-widest text-neutral-500">Max uses (0 = unlimited)</label><input className="input" type="number" min="0" placeholder="0" value={form.maxUses} onChange={(e) => set('maxUses', e.target.value)} /></div>
            <div><label className="mb-1 block text-[11px] font-bold uppercase tracking-widest text-neutral-500">Expiry date</label><input className="input" type="date" value={form.expiresAt} onChange={(e) => set('expiresAt', e.target.value)} /></div>
          </div>
      ...
      {list && list.length > 0 && <div className="mb-4"><div className="relative max-w-sm"><Search size={14} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-neutral-400" /><input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search codes…" className="input !w-full !py-2.5 !pl-9 !text-[13px]" /></div></div>}
      {!list ? <div className="skeleton h-40 rounded-2xl" /> : list.length === 0 ? <div className="grid place-items-center rounded-2xl border border-dashed border-neutral-200 bg-white py-16 text-center"><BadgePercent size={32} className="mb-3 text-neutral-300" /><p className="text-[14px] font-medium text-neutral-700">No discount codes yet</p><p className="mt-1 text-[12px] text-neutral-500">Create your first code, e.g. WELCOME10.</p><button onClick={openNew} className="mt-4 inline-flex items-center gap-1.5 rounded-full bg-neutral-900 px-4 py-2 text-[12px] font-semibold text-white hover:bg-black"><Plus size={13} /> Create code</button></div> : filtered.length === 0 ? <div className="grid place-items-center rounded-2xl border border-dashed border-neutral-200 bg-white py-16 text-center"><Search size={28} className="mb-3 text-neutral-300" /><p className="text-[14px] font-medium text-neutral-700">No codes match "{q}"</p></div> : (
