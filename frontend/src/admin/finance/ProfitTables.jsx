    ...
    <section>
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[520px] border-collapse text-[12px]">
          <thead>
            <tr className="border-b border-[var(--od-border-light)]">
          ...
          <tbody>
            {!rows ? (
              <tr><td colSpan={6} className="py-8 text-center text-[var(--adm-label)]">Loading…</td></tr>
            ) : rows.length === 0 ? (
              <tr><td colSpan={6} className="py-8 text-center text-[var(--adm-label)]">No sales in this range.</td></tr>
            ) : rows.map((r) => (
              <tr key={r.name} className="border-b border-[var(--od-border-light)]">
                <td className="py-2.5">
                  <div className="flex items-center gap-2.5">
                    {r.image && <Img src={r.image} alt="" className="h-9 w-7 shrink-0 border border-[var(--od-border-light)] object-cover" />}
                    <span className="line-clamp-2 max-w-[220px] text-[var(--od-text)]">{r.name}</span>
                  </div>
                </td>
                <td className="py-2.5 text-right tabular-nums text-[var(--adm-label)]">{r.units}</td>
                <td className="py-2.5 text-right tabular-nums text-[var(--od-text)]">{r.revenue.toLocaleString()}</td>
                <td className="py-2.5 text-right tabular-nums text-[var(--adm-label)]">{r.cogs.toLocaleString()}</td>
                <td className="py-2.5 text-right tabular-nums text-[var(--od-text)]">{r.profit.toLocaleString()}</td>
                <td className="py-2.5 text-right tabular-nums text-[var(--od-text)]">{r.margin}%</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <table className="w-full min-w-[520px] border-collapse text-[12px]">
          ...
          <tbody>
            {!rows ? (
              ...
              <tr><td colSpan={6} className="py-8 text-center text-[var(--adm-label)]">No customers in this range.</td></tr>
            ) : rows.map((r) => (
              <tr key={r.phone} className="border-b border-[var(--od-border-light)]">
                <td className="py-2.5">
                  <div className="min-w-0">
                    <p className="truncate text-[var(--od-text)]">
                      {r.atRisk ? <span className="mr-2 text-[var(--adm-label)]">●</span> : null}
                      {r.name}
                    </p>
                    <p className="truncate text-[11px] text-[var(--adm-label)]">{r.city}</p>
                  </div>
                </td>
                <td className="py-2.5 text-right tabular-nums text-[var(--adm-label)]">{r.orders}</td>
                <td className="py-2.5 text-right tabular-nums text-[var(--od-text)]">{r.revenue.toLocaleString()}</td>
                <td className="py-2.5 text-right tabular-nums text-[var(--od-text)]">
                  {r.profit < 0 ? `↓ ${r.profit.toLocaleString()}` : r.profit.toLocaleString()}
                </td>
                <td className="py-2.5 text-right tabular-nums text-[var(--od-text)]">{r.revenue > 0 ? `${r.margin}%` : '—'}</td>
                <td className="py-2.5 text-right tabular-nums text-[var(--adm-label)]">
...
export function CodExposure({ bare = false } = {}) {
  ...
  return (
    <section>
      ...
      <p className="mt-2 text-[12px] text-[var(--adm-label)]">
        across {d.orders} open order{d.orders === 1 ? '' : 's'} · avg {pkr(d.avgExposure)}
      </p>

      <div className="adm-divide-x mt-5 grid grid-cols-2 border-y border-[var(--od-border-light)]">
        <div className="px-4 py-4">
          <p className="adm-label">Not shipped yet</p>
          <p className="adm-metric mt-2 text-[20px] text-[var(--od-text)]">{d.buckets.notShipped}</p>
          <p className="mt-1 text-[11px] text-[var(--adm-label)]">Cancellable at no cost</p>
        </div>
        <div className="px-4 py-4">
          <p className="adm-label">With the courier</p>
          <p className="adm-metric mt-2 text-[20px] text-[var(--od-text)]">{d.buckets.inTransit}</p>
          <p className="mt-1 text-[11px] text-[var(--adm-label)]">{pkr(d.sunkCost)} already spent</p>
        </div>
      </div>

      {d.oldestDays > 5 && (
        <p className="mt-4 text-[13px] leading-relaxed text-[var(--adm-label)]">
        ...
        <div className="mt-5">
          ...
          <ul>
            {d.risky.slice(0, 4).map((r) => (
              <li key={r.phone} className="flex items-center justify-between gap-2 border-b border-[var(--od-border-light)] py-2 text-[13px]">
                <span className="min-w-0 truncate text-[var(--od-text)]">{r.name}</span>
                <span className="shrink-0 text-[11px] tabular-nums text-[var(--adm-label)]">
                  {r.failed}/{r.orders} failed · {r.failRate}%
                </span>
              </li>
            ))}
          </ul>
          <p className="mt-3 text-[12px] leading-relaxed text-[var(--adm-label)]">
...
export function BreakEven({ days, from, to, bare = false }) {
  ...
  return (
    <section>
      ...
      {unset ? (
        <p className="mt-5 border-y border-[var(--od-border-light)] py-6 text-[13px] leading-relaxed text-[var(--adm-label)]">
        ...
        <>
          <div className="mt-5 flex items-baseline gap-2">
            <p className="adm-metric text-[32px] leading-none text-[var(--od-text)]">{d.ordersNeededPerDay}</p>
            <p className="text-[12px] text-[var(--adm-label)]">order{d.ordersNeededPerDay === 1 ? '' : 's'} / day needed</p>
          </div>

          <div className="mt-4 flex flex-wrap items-center gap-3">
            <MonoStatus label={d.onTrack ? 'COVERING COSTS' : 'BELOW BREAK-EVEN'} dim={!d.onTrack} />
            <span className="text-[12px] text-[var(--adm-label)]">running at {d.currentPerDay}/day</span>
          </div>

          <dl className="mt-5 space-y-2 border-t border-[var(--od-border-light)] pt-4 text-[13px]">
            {[
              ...
              <div key={k} className="flex justify-between gap-3">
                <dt className="text-[var(--adm-label)]">{k}</dt>
                <dd className="shrink-0 tabular-nums text-[var(--od-text)]">{v}</dd>
              </div>
            ))}
          </dl>
        </>
