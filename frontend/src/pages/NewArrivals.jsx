 ...
 *   · product count + Filters (opens the full filter sheet — functionality
 *     preserved)
 *   · luxury 4-column grid of LuxuryProductCard (locked design) with two
 *     full-bleed campaign BANNER tiles spanning 2 columns, interleaved
 *   · Show More (reveals +8)
 *   · newsletter sign-up + breadcrumb footer
 * ========================================================================== */
...
export default function NewArrivalsPage() {
  ...
  /* Interleave the two banner tiles into the grid (after 2 and after 6) */
  const nodes = useMemo(() => {
    const out = [];
  ...
  return (
    <div className="w-full bg-[#FAF8F5] px-6 pb-24 pt-28 font-sans text-[#111111] lg:px-12">
      ...
      {/* ── Grid (products + interleaved banners) ── */}
      {products === null ? (
        ...
        <div className={`mx-auto grid max-w-[1400px] grid-cols-1 gap-x-6 gap-y-12 sm:grid-cols-2 lg:grid-cols-4 ${pending ? 'opacity-50' : 'opacity-100'} transition-opacity duration-300`}>
          {nodes.map((n) =>
            n.type === 'banner' ? (
              <Link
                key={n.key}
                to={n.banner.href}
                className="group relative col-span-1 aspect-[4/5] overflow-hidden bg-neutral-200 sm:col-span-2"
              >
                <img
                  src={n.banner.image}
                  alt={n.banner.title}
                ...
                <div className="absolute bottom-8 left-8 space-y-2 text-white">
                  <h3 className="font-serif text-xl uppercase tracking-widest lg:text-2xl">
                    {n.banner.title}
                  </h3>
