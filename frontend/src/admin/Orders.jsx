      ...
      <div className="grid gap-4 p-4 pl-5 md:grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)_minmax(0,1fr)_auto] md:items-center">
        ...
        <div className="min-w-0">
          ...
          <div className="mt-2 flex items-center gap-2">
            ...
            <p className="text-[11px] text-neutral-500">{o.items?.length || 0} item{(o.items?.length || 0) === 1 ? '' : 's'} · {fmtDate(o.createdAt)}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-1.5">
          <Link to={`/admin/orders/${o._id}`} className="rounded-lg border border-neutral-200 px-3 py-1.5 text-[11px] font-semibold text-neutral-700 transition hover:bg-neutral-900 hover:text-white">
        ...
        <div className="border-t border-neutral-100 bg-neutral-50/60 p-4 pl-5">
          ...
          {codPendingVerify && (
            <div className="mb-3 rounded-xl border border-amber-200 bg-amber-50 p-3">
              ...
              <p className="mt-1 text-[11px] text-amber-800">Call the customer to confirm the order before shipping. Fake COD orders cost you courier fees.</p>
              <div className="mt-2 flex flex-wrap gap-2">
                ...
                <button disabled={busy} onClick={() => onAction('cancel', o)} className="inline-flex items-center gap-1.5 rounded-lg bg-white px-3 py-1.5 text-[11px] font-semibold text-red-700 shadow-sm ring-1 ring-red-200 hover:bg-red-50">
          ...
          {awaitingPayment && (
            <div className="mb-3 rounded-xl border border-blue-200 bg-blue-50 p-3">
              ...
              <p className="mt-1 text-[11px] text-blue-800">Verify payment in your account, then click below to confirm.</p>
              <div className="mt-2 flex flex-wrap gap-2">
                <button disabled={busy} onClick={() => onAction('mark-paid', o)} className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-600 px-3 py-1.5 text-[11px] font-semibold text-white shadow-sm hover:bg-emerald-700 disabled:opacity-50">
          ...
          {isConfirmed && (
            <div className="mb-3 rounded-xl border border-blue-200 bg-blue-50 p-3">
              ...
              <p className="mt-1 text-[11px] text-blue-800">Print the invoice, pack the items with discreet packaging, then move to next stage.</p>
              <div className="mt-2 flex flex-wrap gap-2">
                <a href={`/admin/orders/${o._id}/invoice`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-lg bg-white px-3 py-1.5 text-[11px] font-semibold text-neutral-800 shadow-sm ring-1 ring-neutral-300 hover:bg-neutral-100">
                  <Printer size={12} /> Print Invoice
                </a>
                <button disabled={busy} onClick={() => onAction('mark-processing', o)} className="inline-flex items-center gap-1.5 rounded-lg bg-neutral-900 px-3 py-1.5 text-[11px] font-semibold text-white shadow-sm hover:bg-neutral-800 disabled:opacity-50">
          ...
          {isProcessing && (
            <div className="mb-3 rounded-xl border border-purple-200 bg-purple-50 p-3">
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                ...
                <button disabled={busy} onClick={() => onAction('mark-ready', o, trackingDraft)} className="inline-flex items-center gap-1.5 rounded-lg bg-neutral-900 px-3 py-1.5 text-[11px] font-semibold text-white shadow-sm hover:bg-neutral-800 disabled:opacity-50">
          ...
          {isReadyToShip && (
            <div className="mb-3 rounded-xl border border-blue-200 bg-blue-50 p-3">
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                <button disabled={busy} onClick={() => onAction('mark-shipped', o)} className="inline-flex items-center gap-1.5 rounded-lg bg-purple-600 px-3 py-1.5 text-[11px] font-semibold text-white shadow-sm hover:bg-purple-700 disabled:opacity-50">
          ...
          {isShipped && (
            <div className="mb-3 rounded-xl border border-purple-200 bg-purple-50 p-3">
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {o.status === 'Shipped' && (
                  <button disabled={busy} onClick={() => onAction('mark-ofd', o)} className="inline-flex items-center gap-1.5 rounded-lg bg-white px-3 py-1.5 text-[11px] font-semibold text-purple-800 shadow-sm ring-1 ring-purple-300 hover:bg-purple-100 disabled:opacity-50">
                    Out for Delivery
                  </button>
                )}
                <button disabled={busy} onClick={() => onAction('mark-delivered', o)} className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-600 px-3 py-1.5 text-[11px] font-semibold text-white shadow-sm hover:bg-emerald-700 disabled:opacity-50">
                  <CheckCircle2 size={12} /> Mark Delivered
                </button>
                <button disabled={busy} onClick={() => onAction('mark-failed', o)} className="inline-flex items-center gap-1.5 rounded-lg bg-white px-3 py-1.5 text-[11px] font-semibold text-red-700 shadow-sm ring-1 ring-red-200 hover:bg-red-50">
                  Failed Delivery
                </button>
              </div>
            </div>
          ...
          {o.status === 'Delivered' && (
            <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3">
              ...
              <p className="mt-1 text-[11px] text-emerald-800">The customer received their order. You can archive or process a return if requested.</p>
            </div>
          )}
        </div>
      )}
          ...
          return (
            ...
            >
              ...
              <div className="flex items-start justify-between">
                ...
                <span className={`rounded-full px-2 py-0.5 text-[11px] font-bold ${active ? 'bg-neutral-900 text-white' : 'bg-neutral-100 text-neutral-700'}`}>{n}</span>
              </div>
              <p className={`mt-3 text-[13px] font-semibold ${active ? 'text-neutral-900' : 'text-neutral-700'}`}>{s.label}</p>
              {s.hint && <p className="mt-0.5 line-clamp-2 text-[10.5px] leading-relaxed text-neutral-500">{s.hint}</p>}
            </button>
          );
        })}
      </div>
      ...
      <div className="space-y-3">
        ...
        {orders !== null && filtered.length === 0 && !err && (
          <div className="grid place-items-center rounded-2xl border border-dashed border-neutral-200 bg-white py-16 text-center">
            ...
            <p className="mt-1 text-[11px] text-neutral-400">Try changing the stage or search terms.</p>
          </div>
        )}
        {err && (
